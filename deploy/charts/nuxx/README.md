...
```sh
helm install nuxx oci://ghcr.io/tkrnkym/nuxx/charts/nuxx --version 0.1.7 \
  --create-namespace --namespace nuxx \
  --set quickstart=true \
  --set postgresql.enabled=true \
  --set redis.enabled=true \
  --set minio.enabled=true \
  --set relayUrl=wss://nuxx.example.com \
...
`NUXX_S3_ENDPOINT` plus autogenerated credentials automatically. No external services required. The `quickstart=true` flag is an
...
Nuxx uses one URL style for both media and Git/CAS object-store requests:
...
`NUXX_S3_ADDRESSING_STYLE`. It renders `s3.region` as `NUXX_S3_REGION` only
...
Store `NUXX_S3_ACCESS_KEY=${{Object Storage.ACCESS_KEY_ID}}` and
`NUXX_S3_SECRET_KEY=${{Object Storage.SECRET_ACCESS_KEY}}` in the Secret named by
...
`NUXX_GIT_CONFORMANCE_PROBE` is enabled (the relay default). A probe failure is
...
autoscaling:
  ...
  websocketMetricName: nuxx_ws_connections_active
...
`nuxx_ws_connections_active` gauge as a pod metric with the name in
...
Schema migrations are embedded in the relay binary via `sqlx::migrate!` and run at startup, gated by `NUXX_AUTO_MIGRATE` (default `true`). Multiple replicas race-safely behind a Postgres advisory lock. `helm upgrade` is the entire upgrade procedure.
...
1. `NUXX_RELAY_PRIVATE_KEY` — relay identity. Rotating it = new identity (federation peers will not recognize the relay).
...
- **Bundled MinIO is eval-only.** The quickstart profile runs an in-cluster
  ...
  `minio.enabled` off and points `s3.endpoint` (or `NUXX_S3_*` in
...
- **Minimal-mode is not yet supported.** The relay's `NUXX_PUBSUB=local` /
...
`oci://ghcr.io/tkrnkym/nuxx/charts/nuxx` by the `helm chart` workflow
...
for f in ci/*-values.yaml tests/fixtures/*-values.yaml; do
  helm template nuxx . -f "$f" >/dev/null && echo "ok: $f"
done
