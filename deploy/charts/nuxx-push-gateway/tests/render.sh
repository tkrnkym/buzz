...
helm lint deploy/charts/nuxx-push-gateway >/dev/null
helm template push deploy/charts/nuxx-push-gateway >"$out"
# Production values must attach push.nuxx.xyz to an explicit Gateway.
production_args=(
  -f deploy/charts/nuxx-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'appAttestAppId=REALTEAM.xyz.nuxx'
...
helm lint deploy/charts/nuxx-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/nuxx-push-gateway "${production_args[@]}" >"$production_out"
...
runtime={'app.kubernetes.io/name':'nuxx-push-gateway','app.kubernetes.io/instance':'push','app.kubernetes.io/component':'runtime'}
...
assert jenv['NUXX_PUSH_RUNTIME_DATABASE_ROLE']['value']=='nuxx_push_gateway_runtime'
...
required={'DATABASE_URL','NUXX_PUSH_APNS_KEY_ID','NUXX_PUSH_APNS_TEAM_ID','NUXX_PUSH_APNS_TOPIC','NUXX_PUSH_GRANT_KEYS','NUXX_PUSH_TOKEN_KEYS','NUXX_PUSH_MAX_GRANT_LIFETIME_SECONDS'}
assert required <= env
...
np=next(x for x in nps if x['metadata']['name']=='push-nuxx-push-gateway')
migration_np=next(x for x in nps if x['metadata']['name']=='push-nuxx-push-gateway-migration')
...
assert 'push.nuxx.xyz' in route['spec']['hostnames']
PY

# Enabling a route without a Gateway attachment must fail schema validation.
if helm template push deploy/charts/nuxx-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/nuxx-push-gateway -f deploy/charts/nuxx-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
helm template push deploy/charts/nuxx-push-gateway \
...
np=next(x for x in xs if x and x.get('kind')=='NetworkPolicy' and x['metadata']['name']=='push-nuxx-push-gateway')
...
if helm template push deploy/charts/nuxx-push-gateway \
  --set podMonitor.enabled=true \
