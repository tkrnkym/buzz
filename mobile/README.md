...
Flutter mobile client for Nuxx.
...
to the **worktree directory name** (`ai.nuxx.mobile.<slug>` on iOS,
`ai.nuxx.mobile.<slug>` on Android) plus a display-only branch label
in the app name (`Nuxx (my-branch)`, or a short SHA when the worktree is
...
- `NUXX_ANDROID_UPLOAD_KEYSTORE_PATH`: path to a CI-vended keystore file
- `NUXX_ANDROID_UPLOAD_KEYSTORE_PASSWORD`
- `NUXX_ANDROID_UPLOAD_KEY_ALIAS`
- `NUXX_ANDROID_UPLOAD_KEY_PASSWORD`
...
a local upload keystore must set `NUXX_ANDROID_RELEASE_SIGNING=external`. That
mode produces an unsigned release bundle and refuses to run if any
`NUXX_ANDROID_UPLOAD_*` value is also set.
