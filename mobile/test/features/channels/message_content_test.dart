...
import 'package:nuxx/features/channels/media_viewer_page.dart';
  ...
  group('MessageContent', () {
    ...
    group('inline formatting', () {
      ...
      testWidgets('renders markdown link', (tester) async {
        await tester.pumpWidget(
          _testable(
            const MessageContent(content: 'Check [Nuxx](https://example.com)'),
          ),
        );

        final allText = _allRichText(tester);
        expect(allText, contains('Nuxx'));
        // Should not show raw markdown syntax.
        expect(allText, isNot(contains('[Nuxx]')));
