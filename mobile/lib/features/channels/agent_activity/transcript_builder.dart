...
final _nuxxToolNames = <String>{..._nuxxReadTools, ..._nuxxWriteTools};

final _nuxxToolNamesByLength = _nuxxToolNames.toList()
  ..sort((a, b) => b.length.compareTo(a.length));

final _nuxxToolTitleAliases = <(RegExp, String)>[
...
String? _findNuxxToolName(String value, bool includeShortNames) {
  final alias = _findNuxxToolAlias(value);
  if (alias != null) return alias;

  final normalized = _normalizeToolNameText(value);
  for (final name in _nuxxToolNamesByLength) {
...
String? _findNuxxToolAlias(String value) {
  final normalizedPhrase = value
      .trim()
      .toLowerCase()
      .replaceAll(RegExp(r'[_-]+'), ' ')
      .replaceAll(RegExp(r'\s+'), ' ');
  for (final (pattern, name) in _nuxxToolTitleAliases) {
...
_parsePromptText(String text) {
  ...
  for (final section in sections) {
    if (section.title.toLowerCase().startsWith('nuxx event')) {
...
({String title, String toolName, String? nuxxToolName}) _extractToolIdentity(
  Map<String, dynamic> update,
) {
  final candidates = _collectToolNameCandidates(update);
  String? knownName;
  for (final c in candidates) {
    knownName = _findNuxxToolName(c, true);
    if (knownName != null) break;
  }
  knownName ??= _findNuxxToolName(_safeJsonEncode(update), false);
...
List<TranscriptItem> buildTranscript(List<ObserverFrame> events) {
  ...
  void upsertTool(
    ...
    final existing = itemsById[id];
    final canonicalNuxxToolName =
        nuxxToolName ?? _findNuxxToolName(toolName, true);
    if (existing is ToolItem) {
      if (!_isGenericToolTitle(title)) {
        existing.title = title;
      }
      if (canonicalNuxxToolName != null) {
        existing.nuxxToolName = canonicalNuxxToolName;
        existing.toolName = canonicalNuxxToolName;
      } else if (existing.nuxxToolName == null &&
    ...
    final item = ToolItem(
      id: id,
      title: title,
      toolName: canonicalNuxxToolName ?? toolName,
      nuxxToolName: canonicalNuxxToolName,
      status: status,
      args: args,
      result: result,
      isError: isError,
