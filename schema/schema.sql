-- Nuxx initial Postgres schema — multi-tenant.
--
...
-- `resolve_host()` (nuxx-core) applies the identical normalization before
...
-- (crates/nuxx-db/src/push.rs) to close the lost-wake race.
...
-- crates/nuxx-db/src/channel.rs), which forces the same total order the
...
-- event rows are no older than `nuxx.created_at_floor` seconds before commit
...
LANGUAGE plpgsql AS $$
DECLARE
    -- COALESCE across both spellings, as migration 0027 installs: a relay
    -- setting only the new name against the pre-0027 function would get NULL
    -- back and the guard would silently degrade to a no-op.
...
CREATE TABLE push_gateway_installations (
    ...
    -- Both spellings: 0031 widened this so pre-rename rows stay valid while the
    -- renamed gateway writes the `nuxx-` values.
    app_profile TEXT NOT NULL CHECK (app_profile IN ('nuxx-ios-production','nuxx-ios-sandbox','buzz-ios-production','buzz-ios-sandbox')),
...
-- crates/nuxx-db/src/replica_fence.rs and migrations/0026). Exactly one row;
