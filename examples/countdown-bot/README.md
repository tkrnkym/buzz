...
A tiny non-AI Nuxx bot example.

The bot is deliberately boring and algorithmic: it listens to one Nuxx channel
...
It demonstrates that Nuxx participants do not have to be LLM agents. Any
...
members list and in Nuxx's mention autocomplete.
...
```bash
NUXX_RELAY_URL=ws://localhost:3000 \
NUXX_CHANNEL_ID=<channel-uuid> \
NUXX_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
NUXX_BOT_AUTH_MODE=standalone \
...
relay. This reuses the same owner-attestation credential path that Nuxx agents
...
NUXX_CHANNEL_ID=<channel-uuid> \
NUXX_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
NUXX_OWNER_PRIVATE_KEY=<owner-or-agent-nsec-or-hex-secret> \
NUXX_BOT_AUTH_MODE=owner-attested \
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```
...
```bash
NUXX_AUTH_TAG='["auth","<owner-pubkey>","","<sig>"]' \
NUXX_BOT_AUTH_MODE=owner-attested \
# plus NUXX_RELAY_URL, NUXX_CHANNEL_ID, NUXX_BOT_PRIVATE_KEY
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```

Relay requirements for this path:

- `NUXX_REQUIRE_RELAY_MEMBERSHIP=true` on closed relays.
- `NUXX_ALLOW_NIP_OA_AUTH=true` so owner-attested non-member bot keys can be
  admitted.
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The Nuxx UI adds that tag when the bot is selected from
