...
| アバターのアップロード（Blossom。URL の直接入力も可） | ✅ |
| カスタム絵文字（NIP-30。各自が kind:30030 を発行し、パレットは全員の和集合） | ✅ |
| リッチテキスト編集（TipTap。送信時に Markdown へ直列化） | ✅ |
| 下書き（チャンネル/スレッド単位。ブラウザ内にのみ保持、14 日で失効） | ✅ |
| タイムラインの仮想スクロール（virtua。日付見出しは追従ピルへ） | ✅ |
| スレッド内の入れ子表示 | 🚧 |

### モックアップのみ（画面はあるがリレー未接続）

デモビルド（`VITE_MOCK_RELAY=1`）でのみサンプルデータが入り、それ以外では
「まだ接続されていません」と表示します。`features/showcase/use-showcase.ts` が
唯一の分岐点です。

| 機能 | 状態 |
|---|---|
| Agents（一覧・状態別/チャンネル別・詳細パネル・チーム） | 🎭 モック |
| Projects（一覧・概要・Issue・PR・ブランチ・アクティビティ） | 🎭 モック |
| Workflows（一覧・実行トレース・承認カード・実行履歴） | 🎭 モック |
| Pulse（すべて/ノート/エージェントのタブ） | 🎭 モック |
| Reminders（期限・スヌーズ） | 🎭 モック |
| Forum（投稿一覧・スレッドパネル・コメント） | 🎭 モック |
| Huddle（通話バー・参加者・ミュート） | 🎭 モック |
| メンバー管理（一覧・ロール・招待リンク） | 🎭 モック |
