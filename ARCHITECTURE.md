...
│                           CLIENTS                                    │
│                                                                      │
│  Human (Nostr app, web, mobile)    Agent (CLI tools via nuxx-cli)    │
│           │                                    │                     │
│           └──────────── WebSocket ─────────────┘                    │
└─────────────────────────────────────────────────────────────────────┘
                               │
...
└──────────┬──────────────┬──────────────────────────────────────────┘
     ...
     ┌──────────────┐
     │  Postgres    │  ← nuxx-search (FTS over the search_tsv
...
```
nuxx-core    (zero I/O — types, verification, filter matching, kind registry)
    │
    ├── nuxx-db          (Postgres: events, channels, tokens, workflows, audit)
    ├── nuxx-auth        (NIP-42, NIP-98, API tokens, scopes, rate limiting)
    ├── nuxx-pubsub      (Redis pub/sub, presence, typing indicators)
    ├── nuxx-search      (Postgres FTS: query, delete)
    ├── nuxx-audit       (hash-chain tamper-evident log)
    └── nuxx-workflow    (YAML-as-code automation engine)
         │
         └── nuxx-relay       (ties everything together — the server)

nuxx-acp            (agent harness — bridges relay @mentions → AI agents via ACP/JSON-RPC)
nuxx-sdk            (typed Nostr event builders — used by nuxx-acp and nuxx-cli)
nuxx-media          (Blossom/S3 media storage)
nuxx-cli            (agent-first CLI)
nuxx-admin          (operator CLI: relay membership + key generation)
nuxx-test-client    (integration test harness + manual CLI)
```

**Key architectural principle:** The relay is the single source of truth. `nuxx-relay` orchestrates all subsystems by calling them directly — it imports `nuxx-db`, `nuxx-auth`, `nuxx-pubsub`, `nuxx-search`, `nuxx-audit`, and `nuxx-workflow`. However, those subsystems are isolated from each other: `nuxx-workflow` never calls `nuxx-pubsub`, `nuxx-search` never calls `nuxx-db`, etc. Cross-subsystem coordination happens only through the relay. In multi-community mode, the relay also owns propagation of `TenantContext`; service crates should receive community-scoped inputs rather than independently deriving tenancy from client-controlled event tags.
...
### nuxx-core — Shared Types and Verification
...
### nuxx-search — Postgres FTS Integration
...
- Permission filtering is **caller's responsibility** — `nuxx-search` returns
...
Real-time voice lives inside `nuxx-relay` (`src/audio/`), not a separate crate. A WebSocket endpoint (`wss://.../huddle/{channel_id}/audio`) authenticates each participant with a NIP-42 challenge, checks channel membership, admits them to an in-memory room, and forwards opaque Opus frames between peers. No external SFU.
...
- Depends on `nuxx-core` (kind constants) and `nuxx-sdk` (relay/REST utilities).

**Does NOT:** persist state.

---

### nuxx-admin — Operator CLI
...
The `nuxx-admin` binary is shipped in the relay Docker image (`/usr/local/bin/nuxx-admin`) and is the recommended way to manage relay membership in production. Use `./run.sh add-member`, `./run.sh remove-member`, and `./run.sh list-members` in Docker Compose deployments.
...
| Concern | Mechanism |
|---------|-----------|
| Schnorr signatures | `verify_event()` in `nuxx-core` — every event verified before storage |
...
`is_private_ip()` in `nuxx-core` covers:
...
Applied in: `nuxx-workflow` (CallWebhook action), `nuxx-core` (shared utility).
...
| 2 | **No rate limiting implementation** | `RateLimiter` trait exists in `nuxx-auth`. Only implementation is `AlwaysAllowRateLimiter` (test stub, gated behind `#[cfg(any(test, feature = "test-utils"))]`). `RateLimitConfig` defines 4 tiers (human, agent-standard, agent-elevated, agent-platform) but none are enforced. |
