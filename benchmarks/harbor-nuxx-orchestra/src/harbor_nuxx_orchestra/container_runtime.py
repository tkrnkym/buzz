"""Run the production Nuxx agent stack inside the Harbor task container.

Each provisioned identity is a full ``nuxx-acp`` → ``nuxx-agent`` →
``nuxx-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``nuxx`` CLI on
PATH) that a Nuxx host gives an agent. The harness stays outside:
...
# Container-side layout for the uploaded Nuxx stack.
REMOTE_ROOT = "/opt/nuxx"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Nuxx agent process cannot be launched or exits early."""
...
class NuxxContainerRuntime:
    """Launch one production Nuxx agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        nuxx_acp_binary: str = "nuxx-acp",
        nuxx_agent_binary: str = "nuxx-agent",
        nuxx_dev_mcp_binary: str = "nuxx-dev-mcp",
        nuxx_cli_binary: str = "nuxx",
        relay_gateway: str = "",
        ...
        self.nuxx_acp_binary = nuxx_acp_binary
        self.nuxx_agent_binary = nuxx_agent_binary
        self.nuxx_dev_mcp_binary = nuxx_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.nuxx_cli_binary = nuxx_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        if not workers:
            raise RuntimeLaunchError("Nuxx orchestration requires at least one worker")
        trial_dir = self.logs_dir / "nuxx"
        ...
        try:
            ...
            # The task arrives exactly as it would in production Nuxx: a
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/nuxx-acp": self.nuxx_acp_binary,
            f"{REMOTE_BIN}/nuxx-agent": self.nuxx_agent_binary,
            f"{REMOTE_BIN}/nuxx-dev-mcp": self.nuxx_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/nuxx-acp')} </dev/null "
    ...
    def _agent_env(
        ...
        *,
        ...
        return {
            **endpoint.env,
            "NUXX_RELAY_URL": trial.relay_ws_url,
            "NUXX_PRIVATE_KEY": credential.nostr_secret_key,
            # Desktop parity: the GUI also sets NOSTR_PRIVATE_KEY on nuxx-acp
            # so nuxx-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "NUXX_AUTH_TAG": credential.nostr_auth_tag,
            "NUXX_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/nuxx-agent",
            "NUXX_ACP_AGENT_ARGS": "",
            "NUXX_ACP_MCP_COMMAND": f"{REMOTE_BIN}/nuxx-dev-mcp",
            "NUXX_ACP_CHANNELS": trial.channel_id,
            "NUXX_ACP_SUBSCRIBE": "mentions",
            "NUXX_ACP_RESPOND_TO": "anyone",
            "NUXX_ACP_NO_MEMORY": "true",
            "NUXX_ACP_SYSTEM_PROMPT_FILE": remote_prompt,
            "NUXX_AGENT_PROVIDER": endpoint.provider,
            "NUXX_AGENT_MODEL": credential.llm_endpoint,
            "NUXX_AGENT_MAX_OUTPUT_TOKENS": str(
                agent_class.generation.max_output_tokens
            ),
            "NUXX_AGENT_MAX_CONTEXT_TOKENS": str(
                agent_class.generation.context_window_tokens
            ),
            "NUXX_AGENT_MAX_ROUNDS": str(
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._nuxx_json(
                trial.user,
                trial,
                "messages",
                "get",
    ...
    # -- Nuxx CLI as the trial user / provisioning identities -------------------
    ...
    async def _nuxx_json(
        self, credential: AgentCredential, trial: TrialHandle, *args: str
    ) -> Any:
        process = await asyncio.create_subprocess_exec(
            self.nuxx_cli_binary,
            *args,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env={
                **os.environ,
                "NUXX_RELAY_URL": self._user_relay_url(trial),
                "NUXX_PRIVATE_KEY": credential.nostr_secret_key,
                "NUXX_AUTH_TAG": credential.nostr_auth_tag,
            },
        )
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"nuxx {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("nuxx returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return NuxxContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Nuxx workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in Nuxx channel `{trial.channel_id}`.",
            (
    ...
    def _reject_identity_overrides(endpoint: EndpointLaunchConfig) -> None:
        forbidden = {
            ...
            "NUXX_AUTH_TAG",
            "NUXX_ACP_CHANNELS",
            "NUXX_ACP_MCP_COMMAND",
            "NUXX_ACP_AGENT_COMMAND",
        }
