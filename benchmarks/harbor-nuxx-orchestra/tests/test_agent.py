...
from harbor_nuxx_orchestra import (
    AgentCredential,
    NuxxOrchestraAgent,
    RuntimeResult,
    TrialHandle,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = NuxxOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = NuxxOrchestraAgent(
        ...
        nuxx_acp_binary="/pinned/nuxx-acp",
        nuxx_agent_binary="/pinned/nuxx-agent",
        nuxx_dev_mcp_binary="/pinned/nuxx-dev-mcp",
        nuxx_cli_binary="/pinned/nuxx",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.nuxx_acp_binary == "/pinned/nuxx-acp"
    assert agent.runtime.nuxx_agent_binary == "/pinned/nuxx-agent"
    assert agent.runtime.nuxx_dev_mcp_binary == "/pinned/nuxx-dev-mcp"
    assert agent.runtime.nuxx_cli_binary == "/pinned/nuxx"
