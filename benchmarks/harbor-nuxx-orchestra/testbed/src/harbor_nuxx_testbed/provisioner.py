"""Channel-per-trial Nuxx provisioning against a local benchmark stack."""
...
def provisioner_from_dict(config: dict[str, object]) -> NuxxTrialProvisioner:
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return NuxxTrialProvisioner(TestbedConfig(**config))


class NuxxTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live Nuxx relay.

    Guarantees (contract PLANS/HARBOR_NUXX_TRIALHANDLE_CONTRACT.md):
    ...
    def _provision(
        ...
        # mirroring production Nuxx, where a human owns the channel their
        # agents work in.
    ...
    def _cli_for(self, credential: AgentCredential) -> NuxxCli:
        return NuxxCli(
