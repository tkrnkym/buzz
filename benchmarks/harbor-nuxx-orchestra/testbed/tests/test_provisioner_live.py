...
Gated by NUXX_TESTBED_LIVE=1 with stack coordinates in the environment:
  NUXX_TESTBED_RELAY_HTTP   (default http://localhost:3000)
  NUXX_TESTBED_RELAY_WS     (default ws://host.docker.internal:3000)
  NUXX_TESTBED_OWNER_KEY    relay owner secret key (hex)
  NUXX_TESTBED_PG_DSN       benchmark Postgres DSN
"""
...
from harbor_nuxx_testbed.nuxx_cli import NuxxCli, NuxxCliError
from harbor_nuxx_testbed.provisioner import (
...
pytestmark = pytest.mark.skipif(
    os.environ.get("NUXX_TESTBED_LIVE") != "1",
    reason="live testbed suite; set NUXX_TESTBED_LIVE=1 with a running stack",
)


@pytest.fixture()
def provisioner() -> NuxxTrialProvisioner:
    owner_key = os.environ.get("NUXX_TESTBED_OWNER_KEY")
    dsn = os.environ.get("NUXX_TESTBED_PG_DSN")
    if not owner_key or not dsn:
        pytest.fail("NUXX_TESTBED_OWNER_KEY and NUXX_TESTBED_PG_DSN are required")
    return NuxxTrialProvisioner(
        TestbedConfig(
            relay_http_url=os.environ.get(
                "NUXX_TESTBED_RELAY_HTTP", "http://localhost:3000"
            ),
            relay_ws_url=os.environ.get(
                "NUXX_TESTBED_RELAY_WS", "ws://host.docker.internal:3000"
            ),
...
def cli_for(provisioner: NuxxTrialProvisioner, credential) -> NuxxCli:
...
def test_create_is_idempotent_and_isolated(provisioner, manifest):
    ...
    try:
        ...
        with pytest.raises(NuxxCliError, match="private"):
    ...
    provisioner.teardown(handle_a)
    with psycopg.connect(os.environ["NUXX_TESTBED_PG_DSN"]) as conn:
        row = conn.execute(
