#!/usr/bin/env python3
"""One-command benchmark: bring up the Nuxx stack in Docker and run it.
...
- A dedicated ``nuxx-benchmark`` compose project reusing the production
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/harbor-nuxx-orchestra/testbed \
        benchmarks/harbor-nuxx-orchestra/scripts/benchmark.py [...]
"""
...
COMPOSE_PROJECT = "nuxx-benchmark"
COMPOSE_FILES = (
...
AGENT_BINARIES = ("nuxx-acp", "nuxx-agent", "nuxx-dev-mcp")
...
def load_state() -> dict[str, str]:
    ...
    from harbor_nuxx_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_nuxx_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        "NUXX_IMAGE": os.environ.get("NUXX_IMAGE", "ghcr.io/tkrnkym/nuxx:main"),
        "NUXX_DOMAIN": "localhost",
        "RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
        "NUXX_MEDIA_BASE_URL": f"http://localhost:{RELAY_HTTP_PORT}/media",
        "NUXX_MEDIA_SERVER_DOMAIN": "localhost",
        "NUXX_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "NUXX_REQUIRE_AUTH_TOKEN": "true",
        "NUXX_REQUIRE_RELAY_MEMBERSHIP": "true",
        "NUXX_ALLOW_NIP_OA_AUTH": "true",
        "NUXX_AUTO_MIGRATE": "true",
        "NUXX_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "nuxx_relay=info,nuxx_db=info,nuxx_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "NUXX_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "NUXX_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
        "POSTGRES_DB": "nuxx",
        "POSTGRES_USER": "nuxx",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "NUXX_S3_ACCESS_KEY": state["s3_access_key"],
        "NUXX_S3_SECRET_KEY": state["s3_secret_key"],
        "NUXX_S3_BUCKET": "nuxx-media",
        "NUXX_HTTP_PORT": str(RELAY_HTTP_PORT),
        "NUXX_PG_HOST_PORT": str(PG_HOST_PORT),
        "NUXX_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return (
        f"postgresql://nuxx:{state['postgres_password']}@127.0.0.1:{PG_HOST_PORT}/nuxx"
    )
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host nuxx CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host nuxx CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "nuxx-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    nuxx-acp → nuxx-agent → nuxx-dev-mcp stack, so the binaries must be
...
def leaderboard_argv(
    ...
    argv += [
        ...
        # NUXX_BENCHMARK_DOCKER_HOST if your engine exposes the host
        # differently.
        "--relay-gateway",
        (
            f"{os.environ.get('NUXX_BENCHMARK_DOCKER_HOST', 'host.docker.internal')}"
            f":{RELAY_HTTP_PORT}"
        ),
        "--n-concurrent",
