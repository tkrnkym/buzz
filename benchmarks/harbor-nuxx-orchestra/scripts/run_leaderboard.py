...
importable:
    ...
    uv run --project benchmarks/harbor-nuxx-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux nuxx-acp/nuxx-agent/nuxx-dev-mcp>
"""
...
AGENT_IMPORT = "harbor_nuxx_orchestra:NuxxOrchestraAgent"
PROVISIONER_FACTORY = "harbor_nuxx_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the Nuxx relay/Postgres provisioner",
    )
    parser.add_argument(
        "--nuxx-bin-dir",
        type=Path,
        default=None,
        help="Directory with the host nuxx CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir",
        type=Path,
        required=True,
        help="Directory with Linux builds of nuxx-acp/nuxx-agent/nuxx-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"nuxx binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --nuxx-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "nuxx_acp_binary": agent_binaries["nuxx-acp"],
        "nuxx_agent_binary": agent_binaries["nuxx-agent"],
        "nuxx_dev_mcp_binary": agent_binaries["nuxx-dev-mcp"],
        "nuxx_cli_binary": binaries["nuxx"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.nuxx_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.nuxx_bin_dir)
