 ...
 *
 * Derives the app's CSS variables from a syntax theme's key colors (bg, fg,
 * comment, git). Detects light vs dark from background luminance and adjusts
 * accordingly.
 *
 * Every function here is pure, which is the point: the palette the reader sees
 * is computed rather than written down, so the rules that compute it are worth
 * pinning with tests. See `adaptive-theme.test.mjs`.
 */
