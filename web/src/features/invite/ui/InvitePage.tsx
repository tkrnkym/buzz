...
import {
  NUXX_RELEASES_URL,
  type NuxxDownloadPlatform,
  detectNuxxDownloadPlatform,
  resolveNuxxDownloadUrlForPlatform,
} from "@/shared/lib/nuxx-download";
...
export function InvitePage({ code }: { code: string }) {
  ...
  const [downloadUrl, setDownloadUrl] = React.useState(NUXX_RELEASES_URL);
  ...
  React.useEffect(() => {
    let active = true;
    detectNuxxDownloadPlatform(navigator).then(async (platform) => {
      if (!active) return;
      if (
      ...
      const url = await resolveNuxxDownloadUrlForPlatform(platform);
