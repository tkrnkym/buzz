...
//! Nuxx instance administration CLI.
//!
...
use nuxx_core::kind::KIND_NIP43_MEMBERSHIP_LIST;
use nuxx_core::tenant::{relay_url_authority, TenantContext};
use nuxx_db::{Db, DbConfig};
use nuxx_pubsub::{EventTopic, PubSubManager};
use tracing::warn;

#[derive(Parser)]
#[command(name = "nuxx-admin", about = "Nuxx instance administration")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    Migrate,
    /// Inspect deployment-wide Nuxx product feedback.
    ProductFeedback {
        #[command(subcommand)]
...
async fn main() {
    ...
    // Redis TLS connection without this. Mirrors nuxx-relay's main().
...
async fn cmd_remove_member(pubkey_arg: String, role_filter: Option<String>) -> Result<i32> {
    ...
    let tenant = resolve_admin_tenant(&db).await?;
    use nuxx_db::relay_members::RemoveResult;
...
async fn publish_membership_list_with_bump(
    ...
    tracing::info!(
        ...
        ts,
        "NIP-43 membership list published by nuxx-admin"
    );
    Ok(())
}
...
///
/// `nuxx-admin` runs inside the relay container (`compose exec relay
/// nuxx-admin …`), so it shares the relay's `RELAY_URL` and resolves the same
...
async fn resolve_admin_tenant(db: &Db) -> Result<TenantContext> {
    ...
    // resolution do (`nuxx_core::tenant::relay_url_authority`): host plus an
    ...
    // the helper keeps nuxx-admin byte-identical to the community startup seeds.
    ...
    let record = db.lookup_community_by_host(&host).await?.ok_or_else(|| {
        anyhow::anyhow!(
            "RELAY_URL host '{host}' is not mapped to a community.\n\
             nuxx-admin operates on the configured relay's community; ensure the \
...
async fn reconcile_channels(relay_key_arg: Option<String>) -> Result<()> {
    use nuxx_core::kind::KIND_NIP29_GROUP_ADMINS;
