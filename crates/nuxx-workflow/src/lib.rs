...
use dashmap::DashMap;
use nuxx_core::kind::{event_kind_u32, is_workflow_execution_kind, KIND_REACTION};
use nuxx_core::tenant::CommunityId;
use nuxx_db::workflow::RunStatus;
use nuxx_db::Db;
...
pub struct WorkflowEngine {
    ...
    pub(crate) workflow_cache:
        moka::sync::Cache<(CommunityId, Uuid), Arc<Vec<nuxx_db::workflow::WorkflowRecord>>>,
}
...
/// Build a [`executor::TriggerContext`] from a [`nuxx_core::StoredEvent`].
///
...
pub fn build_trigger_context(event: &nuxx_core::StoredEvent) -> executor::TriggerContext {
    let kind_u32 = event_kind_u32(&event.event);
...
fn trigger_matches_event(trigger: &TriggerDef, kind_u32: u32) -> bool {
    use nuxx_core::kind::{KIND_REACTION, KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_DIFF};
    match trigger {
...
"#;
    ...
    fn trigger_matches_stream_message() {
        ...
        assert!(trigger_matches_event(
            &trigger,
            nuxx_core::kind::KIND_STREAM_MESSAGE
        ));
        assert!(!trigger_matches_event(
            &trigger,
    ...
    fn schedule_trigger_never_matches_events() {
        ...
        assert!(!trigger_matches_event(
            &trigger,
            nuxx_core::kind::KIND_WORKFLOW_TRIGGERED
        ));
    }
    ...
    fn workflow_execution_kinds_do_not_match_any_trigger() {
        ...
        for kind in nuxx_core::kind::KIND_WORKFLOW_TRIGGERED
            ..=nuxx_core::kind::KIND_WORKFLOW_APPROVAL_DENIED
        {
            assert!(
    ...
    fn make_message_event() -> nuxx_core::StoredEvent {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "hello world")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        nuxx_core::StoredEvent::new(event, Some(Uuid::new_v4()))
    }

    /// Create a reaction event with an `e` tag pointing to a target message.
    fn make_reaction_event() -> (nuxx_core::StoredEvent, String) {
    ...
    fn build_trigger_context_no_channel_id() {
        ...
        let stored = nuxx_core::StoredEvent::new(event, None);
        let ctx = build_trigger_context(&stored);
    ...
    fn test_build_trigger_context_reaction_multiple_e_tags() {
        ...
        let stored = nuxx_core::StoredEvent::new(event, Some(Uuid::new_v4()));
        let ctx = build_trigger_context(&stored);
    ...
    async fn setup_db() -> nuxx_db::Db {
        let database_url = std::env::var("NUXX_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://nuxx:nuxx_dev@localhost:5432/nuxx".to_owned());
    ...
    async fn setup_channel(db: &nuxx_db::Db, creator: &[u8], member: &[u8]) -> (CommunityId, Uuid) {
        ...
        {
            nuxx_db::CreateCommunityWithOwnerResult::Created(rec) => rec.id,
        ...
        db.create_channel_with_id(
            ...
            nuxx_db::channel::ChannelType::Stream,
            nuxx_db::channel::ChannelVisibility::Open,
            None,
            creator,
            None,
        )
        ...
        db.add_member(
            ...
            channel_id,
            member,
            nuxx_db::channel::MemberRole::Member,
            Some(creator),
        )
        .await
        .expect("add member");
    ...
    fn message_event(channel_id: Uuid) -> nuxx_core::StoredEvent {
        let keys = nostr::Keys::generate();
        let event = nostr::EventBuilder::new(nostr::Kind::Custom(9), "hello")
            .sign_with_keys(&keys)
            .expect("sign");
        nuxx_core::StoredEvent::new(event, Some(channel_id))
    }
