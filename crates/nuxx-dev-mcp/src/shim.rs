...
impl Shim {
    pub fn install() -> std::io::Result<Self> {
        let dir = tempfile::Builder::new().prefix("nuxx-dev-mcp-").tempdir()?;
...
fn write_keyfile(shim_dir: &Path, raw: &str) -> Option<KeyInfo> {
    ...
    let keys = match nostr::Keys::parse(raw) {
        ...
        Err(e) => {
            eprintln!(
                "nuxx-dev-mcp: warning: NOSTR_PRIVATE_KEY is set but invalid ({e}); \
    ...
    if write_keyfile_atomic(&keyfile, raw.as_bytes()).is_err() {
        eprintln!(
            "nuxx-dev-mcp: warning: failed to write nostr keyfile; git auth/signing disabled"
        );
        return None;
    }
    let keyfile_path = match keyfile.to_str() {
        ...
        None => {
            eprintln!(
                "nuxx-dev-mcp: warning: tempdir path is not valid UTF-8; git auth/signing disabled"
            );
            return None;
        }
    };
...
/// via nuxx-agent (which clears env), the base is always 0 — composition only
