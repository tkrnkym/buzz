...
use nuxx_core::kind::KIND_PAIRING;
use nuxx_core::pairing::session::PairingSession;
use nuxx_core::pairing::{
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(nuxx_core::pairing::types::AbortReason::SasMismatch)?
        {
