...
//!
//! Defines [`ChannelAccessChecker`] so `nuxx-auth` can enforce access
//! without depending on `nuxx-db` directly.
...
///
/// Implemented by the database layer (`nuxx-db`) in production. The `nuxx-auth`
