...
//!   P4 agentic tool use     — agent + nuxx-dev-mcp writes a file on disk.
//!
...
//! `nuxx-agent` binary with the exact env vars the relay-mesh preset ships.
//!
...
async fn run() -> anyhow::Result<()> {
    ...
    // mode that broke relay-mesh agents when nuxx-agent's default 32768
    // budget met a 32k-context model). 150k output: passes nuxx-agent's own
    ...
    // P4: agentic tool use via nuxx-dev-mcp — write a real file inside the
...
/// Spawn the real nuxx-agent with relay-mesh preset env and drive one ACP
...
async fn agent_chat_in_isolated_home(
    ...
    let agent = match repo_bin("nuxx-agent") {
        Ok(agent) => agent,
