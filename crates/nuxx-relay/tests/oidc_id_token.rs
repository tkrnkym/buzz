...
use nuxx_relay::identity::oidc::{verify_with_jwks, ProviderConfig};
use serde_json::json;

const KID: &str = "test-key-1";
const ISSUER: &str = "https://issuer.test";
const AUDIENCE: &str = "nuxx-client-id";
...
fn a_token_naming_an_unknown_key_is_refused() {
    ...
    assert!(matches!(
        ...
        Err(nuxx_relay::identity::oidc::OidcError::UnknownKeyId(_))
    ));
}
...
fn a_token_with_no_key_id_is_refused() {
    ...
    assert!(matches!(
        verify_with_jwks(&token, &provider(), &jwks()),
        Err(nuxx_relay::identity::oidc::OidcError::MissingKeyId)
    ));
}
...
fn a_token_resigned_with_the_public_key_as_an_hmac_secret_is_refused() {
    ...
    assert!(
        matches!(
            ...
            Err(nuxx_relay::identity::oidc::OidcError::AlgorithmNotAccepted(
                _
            ))
        ),
