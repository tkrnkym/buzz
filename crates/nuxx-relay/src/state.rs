...
use nuxx_auth::{AuthService, Nip98ReplayGuard};
use nuxx_core::tenant::TenantContext;
use nuxx_core::CommunityId;
use nuxx_db::Db;
use nuxx_media::MediaStorage;
use nuxx_pubsub::cache_invalidation::CacheInvalidation;
use nuxx_pubsub::conn_control::ConnControl;
use nuxx_pubsub::rate_limiter::RedisRateLimiter;
use nuxx_pubsub::{PubSubManager, RedisNip98ReplayGuard};
...
pub async fn run_registered_community_connection<Check, CheckFuture, Run, RunFuture>(
    ...
    CheckFuture: Future<Output = Result<bool, nuxx_db::DbError>>,
...
) -> (usize, Vec<(CommunityId, nuxx_db::DbError)>)
where
...
pub struct AppState {
    ...
    pub audit_tx: Option<mpsc::Sender<nuxx_audit::NewAuditEntry>>,
    ...
    /// See `crates/nuxx-conformance/` and `crate::conformance` for the
    /// schema, emitter helpers, and the independent checker.
    pub tracer: Arc<dyn nuxx_conformance::Tracer>,
...
impl AppState {
    ...
    pub fn new(
        ...
        let (audit_tx, mut audit_rx) = mpsc::channel::<nuxx_audit::NewAuditEntry>(1000);
        ...
        let state = Self {
            ...
            // `crates/nuxx-test-client` once those land).
    ...
    pub async fn disconnect_community_clusterwide(
        &self,
        tenant: &TenantContext,
    ) -> Result<usize, nuxx_pubsub::PubSubError> {
        let closed = self
    ...
    pub async fn channel_visibility_cached(
        ...
        community_id: CommunityId,
        channel_id: Uuid,
        prefetched: Option<&nuxx_db::channel::ChannelRecord>,
    ) -> Result<String, nuxx_db::DbError> {
...
async fn log_audit_entry(audit: &nuxx_audit::AuditService, entry: nuxx_audit::NewAuditEntry) {
    let t = std::time::Instant::now();
    if let Err(e) = audit.log(entry).await {
        metrics::counter!("nuxx_audit_log_errors_total").increment(1);
        tracing::error!("Audit log failed: {e}");
    } else {
        metrics::histogram!("nuxx_audit_log_seconds").record(t.elapsed().as_secs_f64());
    }
}
...
mod tests {
    ...
    async fn test_state() -> Arc<AppState> {
        ...
        let media_storage = nuxx_media::MediaStorage::new(&config.media).expect("media storage");
    ...
    fn shared_counter_between_direct_and_fanout() {
        ...
        let conn = ConnectionState {
            conn_id,
            tenant: nuxx_core::tenant::TenantContext::resolved(
                nuxx_core::tenant::CommunityId::from_uuid(Uuid::nil()),
    ...
    async fn revalidation_continues_after_one_community_lookup_failure() {
        ...
        let (closed, failures) =
            revalidate_registered_communities(&registry, |community| async move {
                if community == failed {
                    Err(nuxx_db::DbError::InvalidData(
    ...
    async fn disconnect_pubkey_is_fenced_to_the_banning_community() {
        ...
        let community_a = nuxx_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xa));
        let community_b = nuxx_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xb));
    ...
    async fn drain_all_sends_restart_close_and_cancels_every_conn() {
        ...
        let (mut ctrl_a, cancel_a) = register(nuxx_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xa),
        ));
        let (mut ctrl_b, cancel_b) = register(nuxx_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xb),
        ));
