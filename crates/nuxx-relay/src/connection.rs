...
use nostr::Filter;
use nuxx_auth::{generate_challenge, AuthContext, LimitType};
...
async fn handle_active_connection(
    ...
    metrics::gauge!("nuxx_ws_connections_active").increment(1.0);
        ...
        tokio::select! {
            _ = tokio::time::sleep(AUTH_TIMEOUT) => {
                ...
                if !authenticated {
                    ...
                    metrics::counter!("nuxx_ws_auth_timeouts_total").increment(1);
    ...
    metrics::gauge!("nuxx_ws_connections_active").decrement(1.0);
    ...
    loop {
        ...
        tokio::select! {
            ...
            Some(msg) = data_rx.recv() => {
                ...
                metrics::histogram!("nuxx_ws_send_batch_size").record(batched as f64);
            }
        }
    }
}
...
fn send_admission_result(
    ...
    match result {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("nuxx_admission_rejections_total", "transport" => "websocket", "reason" => "quota").increment(1);
            conn.send(request_rejection_message(
                sub_id,
        ...
        Err(crate::admission::AdmissionError::Unavailable) => {
            metrics::counter!("nuxx_admission_rejections_total", "transport" => "websocket", "reason" => "unavailable").increment(1);
            conn.send(request_rejection_message(
                sub_id,
