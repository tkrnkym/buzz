...
/// - `NUXX_HOOK_URL` — internal policy endpoint (http://127.0.0.1:{port}/internal/git/policy)
/// - `NUXX_HOOK_SECRET` — per-push HMAC secret
/// - `NUXX_REPO_ID` — repo identifier (d-tag)
/// - `NUXX_COMMUNITY_ID` — server-resolved community UUID for the git HTTP request
/// - `NUXX_PUSHER_PUBKEY` — authenticated pusher's hex pubkey
///
...
# Nuxx pre-receive hook — FAIL-CLOSED
...
: "${NUXX_REPO_ID:?error: NUXX_REPO_ID not set}"
: "${NUXX_REPO_OWNER:?error: NUXX_REPO_OWNER not set}"
: "${NUXX_COMMUNITY_ID:?error: NUXX_COMMUNITY_ID not set}"
: "${NUXX_PUSHER_PUBKEY:?error: NUXX_PUSHER_PUBKEY not set}"
: "${NUXX_HOOK_URL:?error: NUXX_HOOK_URL not set}"
: "${NUXX_HOOK_SECRET:?error: NUXX_HOOK_SECRET not set}"
...
SIGNATURE=$(printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$NUXX_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //')
if [ -z "$SIGNATURE" ]; then
...
SAFE_REPO_ID=$(printf '%s' "$NUXX_REPO_ID" | sed 's/\\/\\\\/g; s/"/\\"/g')
BODY="{\"repo_id\":\"${SAFE_REPO_ID}\",\"repo_owner\":\"${NUXX_REPO_OWNER}\",\"community_id\":\"${NUXX_COMMUNITY_ID}\",\"pusher_pubkey\":\"${NUXX_PUSHER_PUBKEY}\",\"ref_updates\":[${REFS}],\"timestamp\":${TIMESTAMP},\"signature\":\"${SIGNATURE}\"}"
