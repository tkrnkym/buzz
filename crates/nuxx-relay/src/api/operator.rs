...
use nuxx_core::{CommunityId, TenantContext};
...
async fn check_operator_replay(
    ...
    match state
        ...
        .try_mark_in_scope(
            ...
            &event_id,
            nuxx_auth::DEFAULT_REPLAY_TTL_SECS,
        )
        .await
    {
        Ok(true) => Ok(()),
...
pub async fn archive_community(
    ...
    let deployment_host = nuxx_core::tenant::relay_url_authority(&state.config.relay_url);
...
pub async fn transfer_community(
    ...
    let (status, previous_owner) = match result {
        nuxx_db::relay_members::TransferResult::Transferred { previous_owner } => {
            ("transferred", previous_owner)
        }
        nuxx_db::relay_members::TransferResult::AlreadyOwner => ("already_owner", None),
        nuxx_db::relay_members::TransferResult::NoOwner => {
            return Err(api_error(
                StatusCode::NOT_FOUND,
                "community has no owner to transfer from",
            ));
        }
        nuxx_db::relay_members::TransferResult::OwnerConflict => {
            return Err(api_error(
                StatusCode::CONFLICT,
                "owner_conflict: the current owner no longer matches expected_owner_pubkey",
            ));
        }
        nuxx_db::relay_members::TransferResult::LimitReached => {
            return Err(api_error(
                StatusCode::CONFLICT,
...
mod tests {
    ...
    use nuxx_core::{kind::KIND_NIP43_MEMBERSHIP_LIST, CommunityId};
    ...
    async fn fresh_host_at_owner_limit_returns_limit_reached_conflict() {
        ...
        for _ in 0..nuxx_db::relay_members::MAX_COMMUNITIES_PER_OWNER {
            let host = format!("community-{}.example", Uuid::new_v4().simple());
            assert_eq!(
