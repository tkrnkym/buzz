...
use base64::Engine;
use nuxx_audit::{AuditAction, NewAuditEntry};
use nuxx_core::tenant::TenantContext;
use nuxx_media::{BlobDescriptor, MediaError, UploadAttribution, UploadNetworkInfo};
...
fn should_stream_as_video(sniff: &[u8]) -> bool {
    infer::get(sniff).is_some_and(|kind| kind.mime_type() == "video/mp4")
        || nuxx_media::looks_like_iso_bmff(sniff)
}
...
fn upload_rate_limited(
    state: &AppState,
    community_id: nuxx_core::CommunityId,
...
impl FromRequestParts<Arc<AppState>> for AuthenticatedUpload {
    ...
    async fn from_request_parts(
        ...
        nuxx_media::auth::verify_blossom_auth_event(&auth_event, Some(tenant.host()), 3600)?;
        ...
        if upload_rate_limited(state, tenant.community(), &auth_event.pubkey) {
            metrics::counter!("nuxx_media_upload_rejections_total", "reason" => "rate_limit")
                .increment(1);
        ...
        let upload_permit = acquire_upload_permit(state, tenant.community(), &auth_event.pubkey)
            .inspect_err(|_| {
                metrics::counter!("nuxx_media_upload_rejections_total", "reason" => "concurrency")
                    .increment(1);
            })?;
...
async fn upload_attribution(
    ...
    let ip = header_value(&cfg.upload_ip_header).and_then(nuxx_media::parse_public_ip);
    let port = ip.and(header_value(&cfg.upload_port_header).and_then(nuxx_media::parse_port));
...
pub async fn upload_blob(
    ...
    if auth.route_mode == UploadRouteMode::LegacyMedia {
        metrics::counter!("nuxx_media_legacy_upload_route_total").increment(1);
    }
    ...
    let mut descriptor = if should_stream_as_video(&sniff) {
        ...
        nuxx_media::process_video_upload(
            &state.media_storage,
            &state.config.media,
            &auth.tenant,
            &auth.auth_event,
        ...
        if is_image {
            ...
            nuxx_media::process_file_upload(
                &state.media_storage,
                &state.config.media,
                &auth.tenant,
                &auth.auth_event,
...
async fn authenticate_media_read(
    ...
    nuxx_media::auth::verify_blossom_get_auth(&auth_event, sha256, Some(tenant.host()), 3600)?;
...
pub(crate) async fn serve_blob_for_tenant(
    ...
    let disposition = if nuxx_media::serve_inline(&content_type) {
        "inline"
    } else {
        "attachment"
    };
...
async fn resolve_s3_key(
    storage: &nuxx_media::MediaStorage,
    tenant: &TenantContext,
    sha256_ext: &str,
