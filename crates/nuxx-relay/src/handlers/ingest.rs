...
fn emit_product_feedback_success(
    tracer: &Arc<dyn nuxx_conformance::Tracer>,
...
fn required_scope_for_kind(kind: u32, event: &Event) -> Result<Scope, &'static str> {
    match kind {
        ...
        k if nuxx_core::kind::is_moderation_command_kind(k) => Ok(Scope::MessagesWrite),
        ...
        // NIP-43: relay membership admin commands (9030–9032) + Nuxx
...
pub(crate) async fn check_channel_membership(
    ...
    ch_id: Uuid,
    pubkey_bytes: &[u8],
    channel: Option<&nuxx_db::channel::ChannelRecord>,
) -> Result<(), String> {
    match state
...
impl ThreadMetadataOwned {
    pub fn as_params(&self) -> nuxx_db::event::ThreadMetadataParams<'_> {
        nuxx_db::event::ThreadMetadataParams {
...
/// be silently skipped by `validate_and_decrypt`. This mirrored the validator in
/// `nuxx-pair-relay`, which has since been removed; the checks below are now the
/// only copy.
...
/// `crates/nuxx-relay/src/conformance/mod.rs` and
...
async fn ingest_event_inner(
    ...
    if nuxx_core::kind::is_relay_only_kind(kind_u32) {
    ...
    if nuxx_core::kind::is_command_kind(kind_u32) {
    ...
    if nuxx_core::kind::is_moderation_command_kind(kind_u32) {
    ...
    if !nuxx_core::kind::is_moderation_command_kind(kind_u32) && !is_relay_admin_kind(kind_u32) {
        match state
            .db
    ...
    if let Some(ch_id) = channel_id {
        ...
        if !skip_membership {
            ...
            // .community(), …)` call (see crates/nuxx-relay/src/handlers
            // /ingest.rs:424).
            let auth_result =
    ...
    if kind_u32 == KIND_NIP43_LEAVE_REQUEST {
        ...
        match remove_result {
            nuxx_db::relay_members::RemoveResult::Removed => {}
            nuxx_db::relay_members::RemoveResult::NotFound => {
                return Err(IngestError::Rejected(
                    "invalid: you are not a relay member".into(),
                ));
            }
            nuxx_db::relay_members::RemoveResult::IsOwner => {
                return Err(IngestError::Rejected(
                    "invalid: relay owner cannot leave".into(),
                ));
            }
            nuxx_db::relay_members::RemoveResult::RoleMismatch => {
    ...
    if kind_u32 == KIND_NIP29_CREATE_GROUP {
        ...
        if create_name
            .as_ref()
            .map(|n| {
                nuxx_core::channel::canonical_channel_name(n)
                    .trim()
                    .is_empty()
            })
            .unwrap_or(true)
        ...
        let visibility: nuxx_db::channel::ChannelVisibility = visibility_str
            .parse()
            .map_err(|_| IngestError::Rejected(format!("invalid visibility: {visibility_str}")))?;
        let channel_type: nuxx_db::channel::ChannelType =
        ...
        if let Some(client_uuid) = channel_id {
            ...
            let name = nuxx_core::channel::canonical_channel_name(&name);
    ...
    if kind_u32 == super::push_lease::KIND_PUSH_LEASE {
        ...
        match outcome {
            nuxx_db::push::AcceptLeaseOutcome::Accepted => {}
            nuxx_db::push::AcceptLeaseOutcome::StaleEvent => {
                return Err(IngestError::Rejected("invalid: stale replacement".into()));
            }
            nuxx_db::push::AcceptLeaseOutcome::StaleGeneration => {
                return Err(IngestError::Rejected("invalid: stale generation".into()));
            }
            nuxx_db::push::AcceptLeaseOutcome::EndpointAlreadyLeased => {
                return Err(IngestError::Rejected(
                    "invalid: endpoint already leased".into(),
                ));
            }
            nuxx_db::push::AcceptLeaseOutcome::LeaseQuotaExceeded => {
                return Err(IngestError::Rejected(
                    "invalid: lease quota exceeded".into(),
                ));
            }
            nuxx_db::push::AcceptLeaseOutcome::SourceEventCollision => {
                return Err(IngestError::Rejected(
                    "invalid: source event collision".into(),
                ));
            }
            nuxx_db::push::AcceptLeaseOutcome::ConstraintViolation => {
                return Err(IngestError::Rejected(
    ...
    if kind_u32 == KIND_REACTION {
        ...
        let (stored_event, was_inserted) = match state
            ...
            .map_err(|e| IngestError::Internal(format!("error: {e}")))?
        {
            nuxx_db::ReactionEventInsertOutcome::TargetMissing => {
                return Err(IngestError::Rejected(
                    "invalid: reaction target event not found".into(),
                ));
            }
            nuxx_db::ReactionEventInsertOutcome::Duplicate => {
                return Ok(IngestResult {
                    event_id: event_id_hex,
                    accepted: false,
            ...
            nuxx_db::ReactionEventInsertOutcome::Inserted {
                stored_event,
                was_inserted,
    ...
    let (stored_event, was_inserted) = if nuxx_core::kind::is_replaceable(kind_u32) {
        ...
        let d_tag = nuxx_db::event::extract_d_tag(&event).unwrap_or_default();
        if d_tag.len() > nuxx_db::event::D_TAG_MAX_LEN {
            return Err(IngestError::Rejected(format!(
                "invalid: d tag too long ({} bytes, max {})",
                d_tag.len(),
                nuxx_db::event::D_TAG_MAX_LEN,
            )));
        }
        state
            .db
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                return Err(match e {
                    nuxx_db::DbError::AuthEventRejected => {
    ...
    if let Some(ch_id) = channel_id {
        if nuxx_core::kind::is_channel_activity_kind(kind_u32) {
...
mod tests {
    ...
    use nuxx_conformance::{TraceStep, Tracer};
    use nuxx_core::kind::{
    ...
    fn feedback_success_action_satisfies_ingest_emit_guard() {
        let community = nuxx_core::CommunityId::from_uuid(Uuid::new_v4());
    ...
    fn make_agent_turn_metric(
        ...
        nostr::EventBuilder::new(
            nostr::Kind::Custom(nuxx_core::kind::KIND_AGENT_TURN_METRIC as u16),
            content,
        )
        .tags(nostr_tags)
