...
use nuxx_core::kind::{KIND_IA_ARCHIVE_REQUEST, KIND_IA_UNARCHIVE_REQUEST, KIND_PROFILE};
...
fn verify_auth_tag_owner(auth_tag_json: &str, target_hex: &str) -> Result<String, String> {
    let target_pubkey =
        PublicKey::from_hex(target_hex).map_err(|e| format!("invalid target pubkey: {e}"))?;
    nuxx_sdk::nip_oa::verify_auth_tag(auth_tag_json, &target_pubkey)
...
mod tests {
    ...
    fn auth_tag(owner_keys: &Keys, target_pubkey: &nostr::PublicKey) -> Tag {
        let tag_json = nuxx_sdk::nip_oa::compute_auth_tag(owner_keys, target_pubkey, "")
            .expect("compute auth tag");
        nuxx_sdk::nip_oa::parse_auth_tag(&tag_json).expect("parse auth tag")
    }
    ...
    async fn seed_test_community(pool: &sqlx::PgPool) -> nuxx_core::tenant::TenantContext {
