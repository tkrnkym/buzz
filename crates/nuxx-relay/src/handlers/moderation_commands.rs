...
//! [`nuxx_core::kind::is_moderation_command_kind`], list them in
...
//!
//! Lane ownership: L6 (Quinn) — plus `nuxx-cli` `moderation` command group.
...
use nuxx_db::moderation::NewAction;
...
fn ensure_actor_not_banned(
    restriction: &nuxx_db::moderation::RestrictionState,
) -> Result<(), String> {
...
async fn handle_resolve(
    ...
    let (target_pubkey, target_event_id) = match &report.target {
        nuxx_db::moderation::ReportTarget::Pubkey(p) => (Some(p.as_slice()), None),
        nuxx_db::moderation::ReportTarget::Event(e) => (None, Some(e.as_slice())),
        nuxx_db::moderation::ReportTarget::Blob(_) => (None, None),
    };
...
mod tests {
    ...
    fn banned_admin_cannot_reach_an_unban_command() {
        let banned = nuxx_db::moderation::RestrictionState {
            banned: true,
            muted_until: None,
        };
        assert_eq!(
        ...
        let timed_out = nuxx_db::moderation::RestrictionState {
            banned: false,
    ...
    fn resolve_audit_actions_are_allowed_by_db_check_vocabulary() {
        for action in ["dismiss", "escalate", "delete", "kick", "ban", "timeout"] {
            let audit_action = resolution_audit_action(action);
            assert!(
                nuxx_db::moderation::MODERATION_ACTION_CHECK_VOCAB.contains(&audit_action),
