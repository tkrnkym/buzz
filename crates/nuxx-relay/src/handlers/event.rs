...
use nuxx_core::activity::ActivityShard;
use nuxx_core::event::StoredEvent;
use nuxx_core::kind::{
...
use nuxx_core::observer::{
    content_looks_like_nip44, OBSERVER_AGENT_TAG, OBSERVER_FRAME_CONTROL, OBSERVER_FRAME_TAG,
    OBSERVER_FRAME_TELEMETRY,
};
use nuxx_core::tenant::TenantContext;
use nuxx_core::verification::verify_event;
use nuxx_core::CommunityId;
use nuxx_pubsub::EventTopic;
...
pub async fn filter_fanout_by_access(
    ...
    let matches = if nuxx_core::kind::is_persona_shared_kind(event_kind_u32(&stored_event.event)) {
        let author = stored_event.event.pubkey.to_bytes();
        matches
            .into_iter()
            .filter(|(conn_id, _)| {
...
pub async fn fan_out_pubsub_event(state: &Arc<AppState>, channel_event: nuxx_pubsub::ChannelEvent) {
    ...
    let channel_id = match channel_event.topic {
        nuxx_pubsub::EventTopic::Channel(id) => Some(id),
        nuxx_pubsub::EventTopic::Global => None,
    };
...
async fn dispatch_persistent_event_inner(
    ...
    // ingest seam (`crates/nuxx-relay/src/handlers/ingest.rs`'s
    ...
    let owner_only_kind = kind_u32 == nuxx_core::kind::KIND_DM_VISIBILITY
        || kind_u32 == nuxx_core::kind::KIND_AGENT_TURN_METRIC;
...
async fn enqueue_event_created_audit(
    ...
    let audit_entry = nuxx_audit::NewAuditEntry {
        community_id: tenant.community(),
        action: nuxx_audit::AuditAction::EventCreated,
    ...
    if let Err(e) = audit_tx.send(audit_entry).await {
        ...
        metrics::counter!("nuxx_audit_send_errors_total").increment(1);
    }
}
...
pub async fn handle_event(event: Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
    ...
    if kind_u32 == nuxx_core::kind::KIND_AUTH {
        reject("invalid");
        conn.send(RelayMessage::ok(
            &event_id_hex,
            false,
    ...
    if kind_u32 == KIND_AGENT_OBSERVER_FRAME {
        if !scopes.is_empty() && !scopes.contains(&nuxx_auth::Scope::MessagesWrite) {
            reject("scope");
            conn.send(RelayMessage::ok(
                &event_id_hex,
                false,
...
mod tests {
    ...
    fn snapshot_with_d_tags(d_tags: &[&str]) -> nostr::Event {
        let mut builder = EventBuilder::new(
            Kind::Custom(nuxx_core::kind::KIND_CHANNEL_ACTIVITY_SNAPSHOT as u16),
            "{}",
        );
        for d in d_tags {
    ...
    async fn observer_frame_rate_limiter_is_scoped_by_community() {
        ...
        let community_a = nuxx_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xAAAA));
        let community_b = nuxx_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xBBBB));
    ...
    async fn observer_owner_cache_is_scoped_to_community() {
        ...
        let community_a = nuxx_core::CommunityId::from_uuid(Uuid::new_v4());
        let community_b = nuxx_core::CommunityId::from_uuid(Uuid::new_v4());
    ...
    mod pubsub_fanout {
        ...
        use nuxx_core::kind::{KIND_MEMBER_ADDED_NOTIFICATION, KIND_PRESENCE_UPDATE};
        use nuxx_pubsub::{ChannelEvent, EventTopic};
        ...
        fn register_global_sub(
            ...
            state.conn_manager.register(
                ...
                nuxx_core::tenant::CommunityId::from_uuid(Uuid::nil()),
        ...
        async fn global_presence_pubsub_event_fans_out_to_local_subscribers() {
            ...
            fan_out_pubsub_event(
                &state,
                ChannelEvent {
                    community_id: nuxx_core::tenant::CommunityId::from_uuid(Uuid::nil()),
        ...
        async fn local_echo_presence_pubsub_event_is_not_delivered_twice() {
            ...
            let community = nuxx_core::tenant::CommunityId::from_uuid(Uuid::nil());
        ...
        async fn local_echo_suppression_is_scoped_to_its_community() {
            ...
            let community_a = nuxx_core::tenant::CommunityId::from_uuid(Uuid::new_v4());
            let community_b = nuxx_core::tenant::CommunityId::from_uuid(Uuid::nil());
        ...
        async fn redis_presence_publish_reaches_second_relay_and_suppresses_origin_echo() {
            ...
            // Match nuxx-pubsub's own Redis round-trip test: give PSUBSCRIBE a
        ...
        async fn audit_records_caller_actor_not_relay_signer_for_relay_signed_event() {
            use nuxx_core::event::StoredEvent;
            use nuxx_core::tenant::{CommunityId, TenantContext};
        ...
        /// call that `nuxx_audit::service::tests::chains_are_independent_per_community`
    ...
    mod fanout_access {
        ...
        pub(super) async fn test_state_with_redis_url(redis_url: &str) -> Arc<AppState> {
            ...
            let media_storage =
                nuxx_media::MediaStorage::new(&config.media).expect("media storage");
        ...
        async fn open_channel_event_passes_through_unfiltered() {
            ...
            let community_id = nuxx_core::tenant::CommunityId::from_uuid(Uuid::nil());
            state
        ...
        async fn author_only_reminder_delivers_to_author_only() {
            ...
            let reminder = EventBuilder::new(
                Kind::Custom(nuxx_core::kind::KIND_EVENT_REMINDER as u16),
                "{}",
            )
    ...
    //
    //  * `ConnEntry` (`crates/nuxx-relay/src/state.rs:30-44`) records
    ...
    //  * `SubscriptionRegistry` (`crates/nuxx-relay/src/subscription.rs:57+`)
    ...
    mod redteam {
        ...
        use nostr::{EventBuilder, Keys, Kind};
        use nuxx_core::kind::KIND_PRESENCE_UPDATE;
        use nuxx_core::StoredEvent;
        use tokio::sync::{mpsc, Mutex};
        use tokio_util::sync::CancellationToken;
        use uuid::Uuid;
        ...
        fn register_conn(
            state: &AppState,
            community_id: nuxx_core::tenant::CommunityId,
            pubkey: Option<Vec<u8>>,
        ) -> Uuid {
            let conn_id = Uuid::new_v4();
            let (tx, _rx) = mpsc::channel(1);
        ...
        async fn channel_less_event_must_drop_recipient_in_different_community() {
            ...
            let community_a = nuxx_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xAAAA));
            let community_b = nuxx_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xBBBB));
