...
use nuxx_core::kind::KIND_STREAM_MESSAGE;
use nuxx_core::tenant::CommunityId;
use nuxx_workflow::action_sink::{ActionSink, ActionSinkError};
use tracing::info;
use uuid::Uuid;
...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let tenant = nuxx_core::tenant::TenantContext::resolved(community_id, host);
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    nuxx_db::DbError::ChannelNotFound(_) | nuxx_db::DbError::NotFound(_) => {
            ...
            //    - `nuxx:workflow` tag prevents recursive workflow triggering
            ...
            let thread_meta = Some(nuxx_db::event::ThreadMetadataParams {
...
mod integration_tests {
    ...
    //!   `cargo test -p nuxx-relay --lib workflow_sink -- --ignored`
    use super::*;
    use nuxx_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use nuxx_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
