...
use nuxx_core::kind::{KIND_MANAGED_AGENT, KIND_TEAM};
...
pub async fn cmd_create_channel(
    ...
    let vis = match visibility {
        "open" => nuxx_sdk::Visibility::Open,
        "private" => nuxx_sdk::Visibility::Private,
        _ => unreachable!(),
    };
    let ct = match channel_type {
        "stream" => nuxx_sdk::ChannelKind::Stream,
        "forum" => nuxx_sdk::ChannelKind::Forum,
        _ => unreachable!(),
    };
    let builder =
        nuxx_sdk::build_create_channel(channel_uuid, name, Some(vis), Some(ct), description, ttl)
...
pub async fn cmd_create_channel_from_template(
    ...
    let builder = nuxx_sdk::build_create_channel(
        channel_uuid,
        name,
        Some(vis),
        Some(ct),
    ...
    if let Some(canvas_template) = template.canvas_template.as_deref() {
        ...
        let canvas_result: Result<(), CliError> = async {
            let builder = nuxx_sdk::build_set_canvas(channel_uuid, &content)
    ...
    for agent in &resolved.agents {
        let outcome: Result<(), CliError> = async {
            let builder = nuxx_sdk::build_add_member(
                channel_uuid,
                &agent.pubkey,
                Some(nuxx_sdk::MemberRole::Bot),
            )
...
pub async fn cmd_update_channel(
    ...
    let builder = nuxx_sdk::build_update_channel(channel_uuid, name, description, None, ttl_change)
...
pub async fn cmd_set_channel_topic(
    ...
    let builder = nuxx_sdk::build_set_topic(channel_uuid, topic)
...
pub async fn cmd_set_channel_purpose(
    ...
    let builder = nuxx_sdk::build_set_purpose(channel_uuid, purpose)
...
pub async fn cmd_add_channel_member(
    ...
    let typed_role = match role {
        None => None,
        Some("owner") => Some(nuxx_sdk::MemberRole::Owner),
        Some("admin") => Some(nuxx_sdk::MemberRole::Admin),
        Some("member") => Some(nuxx_sdk::MemberRole::Member),
        Some("guest") => Some(nuxx_sdk::MemberRole::Guest),
        Some("bot") => Some(nuxx_sdk::MemberRole::Bot),
        Some(other) => {
            return Err(CliError::Usage(format!(
    ...
    let builder = nuxx_sdk::build_add_member(channel_uuid, pubkey, typed_role)
        .map_err(|e| CliError::Other(format!("build_add_member failed: {e}")))?;
...
pub async fn cmd_remove_channel_member(
    ...
    let builder = nuxx_sdk::build_remove_member(channel_uuid, pubkey)
...
pub async fn cmd_set_add_policy(client: &NuxxClient, policy: &str) -> Result<(), CliError> {
    ...
    let builder = EventBuilder::new(
        Kind::Custom(nuxx_sdk::kind::KIND_AGENT_PROFILE as u16),
        &content,
    )
    .tags([]);
    let event = client.sign_event(builder)?;
