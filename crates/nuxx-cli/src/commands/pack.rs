...
pub fn cmd_validate(path: &str) -> Result<(), CliError> {
    ...
    let report = nuxx_persona::validate::validate_pack(pack_dir);

    for diag in &report.diagnostics {
        match diag {
            nuxx_persona::validate::ValidationDiagnostic::Error(msg) => {
                eprintln!("  ERROR: {msg}");
            }
            nuxx_persona::validate::ValidationDiagnostic::Warning(msg) => {
                eprintln!("  WARN:  {msg}");
            }
        }
    }
...
pub fn cmd_inspect(path: &str) -> Result<(), CliError> {
    ...
    let pack = nuxx_persona::resolve::resolve_pack(pack_dir)
