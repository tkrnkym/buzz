...
fn relay_server_tag(relay_url: &str) -> Option<String> {
    let authority = nuxx_core::tenant::relay_url_authority(relay_url);
...
impl BuzzClient {
    ...
    ///
    /// - `NUXX_CONNECT_TIMEOUT_SECS` — TCP connect timeout (default 15 s)
    /// - `NUXX_TIMEOUT_SECS` — per-request total timeout (default 30 s)
    ///
    ...
    pub fn new(
        ...
        let http = reqwest::Client::builder()
            .timeout(env_duration_secs("NUXX_TIMEOUT_SECS", 30))
            .connect_timeout(env_duration_secs("NUXX_CONNECT_TIMEOUT_SECS", 15))
            .build()
                ...
                Ok(resp) => {
                    ...
                    if !resp_was_success(status) {
                        let message = serde_json::from_str::<serde_json::Value>(&body_text)
                            ...
                            .unwrap_or(body_text);
                        let message = if status == 403 && std::env::var("NUXX_AUTH_TAG").is_ok() {
                            format!(
                                "{message} (NUXX_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                            )
                        } else {
                            message
                        };
    ...
    /// `nuxx_ws_client::publish_event` which handles connect, NIP-42 auth,
    ...
    pub async fn publish_ephemeral_event(&self, event: nostr::Event) -> Result<String, CliError> {
        ...
        // See nuxx_ws_client::{AUTH_CHALLENGE_TIMEOUT_SECS, AUTH_OK_TIMEOUT_SECS,
        // PUBLISH_OK_TIMEOUT_SECS} for the inner ceilings.
        let ok =
            nuxx_ws_client::publish_event(&ws_url, event, &self.keys, self.auth_tag.as_ref(), 75)
                .await
                .map_err(|e| CliError::Other(e.to_string()))?;
    ...
    async fn handle_response(&self, resp: reqwest::Response) -> Result<String, CliError> {
        if !resp.status().is_success() {
            ...
            let message = serde_json::from_str::<serde_json::Value>(&body)
                ...
                .unwrap_or(body);
            if status == 403 && std::env::var("NUXX_AUTH_TAG").is_ok() {
                let message = format!(
                    "{message} (NUXX_AUTH_TAG is set — it may be stale or revoked; try unsetting it)"
                );
                return Err(CliError::Relay {
...
/// NUXX_RELAY_URL may be ws/wss (copied from MCP config).
...
mod retry_tests {
    ...
    fn env_duration_secs_parsing() {
        ...
        const KEY: &str = "NUXX_CLI_TEST_DURATION_SECS";
