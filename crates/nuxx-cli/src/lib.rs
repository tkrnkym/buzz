...
///
/// ```ignore
/// let code = nuxx_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from nuxx-acp/nuxx-dev-mcp)
    ...
    // nuxx-dev-mcp delegates to run_from_args, it has already installed ring; the
...
Configuration (flags override env vars):
  ...
  NUXX_PRIVATE_KEY   Nostr private key (hex or nsec)  [required]
  NUXX_AUTH_TAG      NIP-OA auth tag JSON  [optional]
...
struct Cli {
    /// Relay URL (http:// or https://). Overrides NUXX_RELAY_URL env var.
    #[arg(long, env = "NUXX_RELAY_URL", default_value = "http://localhost:3000")]
    relay: String,
    ...
    #[arg(long, env = "NUXX_AUTH_TAG", hide_env_values = true)]
...
running under NUXX_AUTH_TAG signs as itself, so it can only ever satisfy \
...
pub enum ReposCmd {
    ...
    ///
    /// The `nuxx-channel` tag on the announcement is the git ACL: the relay
...
pub enum MemCmd {
    ...
    Ls {
        /// Owner pubkey (hex). Overrides NUXX_AUTH_TAG.
        #[arg(long)]
        owner: Option<String>,
...
/// `NUXX_RELAY_URL` — moderation commands are community-global and carry no
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let private_key_str = cli.private_key.ok_or_else(|| {
        CliError::Auth("NUXX_PRIVATE_KEY is required (use --private-key or set env var)".into())
    })?;
    let keys = Keys::parse(&private_key_str)
        .map_err(|e| CliError::Key(format!("invalid NUXX_PRIVATE_KEY: {e}")))?;
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref json) if !json.is_empty() => {
            let tag = nuxx_sdk::nip_oa::parse_auth_tag(json)
                .map_err(|e| CliError::Auth(format!("NUXX_AUTH_TAG is malformed: {e}")))?;
            nuxx_sdk::nip_oa::verify_auth_tag(json, &keys.public_key()).map_err(|e| {
                CliError::Auth(format!(
                    "NUXX_AUTH_TAG verification failed for pubkey {}: {e}",
