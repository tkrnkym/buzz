...
use nuxx_core::{CommunityId, StoredEvent};
            ...
            ReadSessionInner::Replica { tx, writer } => {
                match event::query_events_on(tx, q).await {
                    ...
                    Err(e) => {
                        ...
                        // Deliberately not a `nuxx_db_route_decision` event:
                        // the page's route was already recorded, and the
                        // offload metric must stay one-event-per-request.
                        metrics::counter!("nuxx_db_read_session_degraded").increment(1);
                        writer.clone()
                    }
                }
            }
...
enum RouteDecision {
    ...
    /// failure emits exactly one `nuxx_db_route_decision` event per request
...
impl Db {
    ...
    > {
        ...
        .await
        {
            ...
            // `nuxx_db_route_decision{decision="writer",reason="reader_acquire_timeout"}`
            ...
            // Runbook: correlate with `nuxx_db_read_pool_active` / `_max`
    ...
    /// request/connection error. nuxx-db only reads the durable host map.
    ...
    /// fully-formed [`nuxx_core::tenant::TenantContext`] — host included — to
    ...
    ///
    /// Used by the runtime conformance read-seam emitters in `nuxx-relay`:
    ...
    pub async fn replace_addressable_event(
        ...
        let kind_i32 = nuxx_core::kind::event_kind_i32(event);
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![nuxx_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = nuxx_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
    ...
    pub async fn replace_parameterized_event(
        ...
        let read_state_t_tag_count = event
            ...
            .count();
        let is_nip_rs = kind_i32 == nuxx_core::kind::KIND_READ_STATE as i32
            && d_tag_count == 1
            && has_exact_d_tag
...
mod tests {
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let old = EventBuilder::new(Kind::Custom(nuxx_core::kind::KIND_READ_STATE as u16), "old")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old");
        let new = EventBuilder::new(Kind::Custom(nuxx_core::kind::KIND_READ_STATE as u16), "new")
            .tags(tags)
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(nuxx_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
    ...
    async fn duplicate_nip_rs_discriminator_tags_keep_legacy_retention() {
        ...
        for (case, tags) in [
            ...
            let old = EventBuilder::new(
                Kind::Custom(nuxx_core::kind::KIND_READ_STATE as u16),
                format!("{case}-old"),
            )
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        let base = Timestamp::now().as_secs();
        let a = EventBuilder::new(Kind::Custom(nuxx_core::kind::KIND_READ_STATE as u16), "A")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign A");
        let x = EventBuilder::new(Kind::Custom(nuxx_core::kind::KIND_READ_STATE as u16), "X")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign X");
        let b = EventBuilder::new(Kind::Custom(nuxx_core::kind::KIND_READ_STATE as u16), "B")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 2))
            .sign_with_keys(&keys)
            .expect("sign B");
        let c = EventBuilder::new(Kind::Custom(nuxx_core::kind::KIND_READ_STATE as u16), "C")
            .tags(tags)
        ...
        async fn legacy_insert(
            ...
            .bind(nuxx_core::kind::KIND_READ_STATE as i32)
    ...
    /// ceiling, not the writer's — `nuxx_db_read_pool_active / _max` is the
    ...
    /// directly, because `nuxx_db_route_decision` is emitted by `route_read`
    ...
    async fn routed_fallback_spends_one_acquire_budget_when_aurora_cache_is_cold() {
        ...
        let reasons: std::collections::HashMap<(String, String), u64> = snapshotter
            ...
            .into_vec()
            .into_iter()
            .filter(|(key, ..)| key.key().name() == "nuxx_db_route_decision")
            .map(|(key, _, _, value)| {
                let metrics_util::debugging::DebugValue::Counter(n) = value else {
                    panic!("nuxx_db_route_decision must be a counter");
                };
