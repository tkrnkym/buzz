...
//! turn.  Both goose and nuxx-agent use this same wire format.  The payload
...
/// ```
///
/// `used` and `contextLimit` are optional because nuxx-agent does not track a
...
pub(crate) struct UsageUpdatePayload {
    /// Total tokens used (context-usage proxy). Optional — nuxx-agent omits
    /// this field or sends 0 because it does not track a context window limit.
    #[serde(default)]
    #[allow(dead_code)]
    pub used: u64,
    /// Context window size. Optional — nuxx-agent omits this field.
    #[serde(default)]
    #[allow(dead_code)]
    ...
    /// does not send it, and nuxx-agent only reports a non-zero value when the
    ...
    /// emitted by nuxx-agent when every turn in the session so far supplied a
...
pub struct TurnUsage {
    ...
    /// Session-cumulative genuine provider total tokens as reported by nuxx-agent;
...
mod tests {
    ...
    /// The camelCase key nuxx-agent actually puts on the wire must land on the
    ...
    fn notification_deserializes_without_used_and_context_limit() {
        // nuxx-agent emits usage_update without used/contextLimit.
        let raw = serde_json::json!({
            "sessionId": "nuxx-sess",
            "update": {
                "sessionUpdate": "usage_update",
    ...
    #[test]
    fn nuxx_agent_notification_flows_through_tracker() {
        // End-to-end: a nuxx-agent-shaped usage_update (no used/contextLimit)
    ...
    #[test]
    fn nuxx_agent_payload_no_context_fields_processes_correctly() {
