...
//! model, or credentials), it spawns `nuxx-acp` with a setup-mode payload
...
///
/// nuxx-acp is a separate crate and must NOT depend on desktop types —
...
/// (`managed_agents/readiness.rs`). Desktop serializes it; `nuxx-acp`
...
async fn publish_setup_nudge(
    ...
    let event_builder = nuxx_sdk::build_message(
        channel_id,
        &body,
        thread_ref.as_ref(),
...
mod tests {
    ...
    //
    // These tests prove the desktop→nuxx-acp→sentinel path preserves the
    // `availability` field. They simulate what actually happens at runtime:
    // desktop serializes a `cli_login` JSON blob → nuxx-acp parses it via
