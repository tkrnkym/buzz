...
| Group | Key commands |
|-------|-------------|
| `nuxx agents` | `draft-create`, `draft-update` |
| `nuxx messages` | `send`, `get`, `thread`, `search` |
| `nuxx channels` | `list`, `get`, `create`, `join`, `members` |
| `nuxx canvas` | `get`, `set` |
| `nuxx reactions` | `add`, `remove` |
| `nuxx dms` | `list`, `open` |
| `nuxx users` | `get`, `set-profile`, `presence` |
| `nuxx workflows` | `list`, `trigger`, `runs` |
| `nuxx feed` | `get` |
| `nuxx social` | `publish`, `notes` |
| `nuxx repos` | `create`, `get`, `list` |
| `nuxx pr` | `open`, `update`, `get`, `list`, `status` |
| `nuxx upload` | `file` |

Run `nuxx --help` or `nuxx <group> --help` for full usage. For multiline message content, pass real newline bytes through stdin: `printf 'first\n\nsecond\n' | nuxx messages send ... --content -`. Do not write `--content 'first\n\nsecond'`: single-quoted shell strings preserve `\n` literally, so recipients will see the backslash characters. `nuxx agents draft-create` and `nuxx agents draft-update` require `NUXX_AUTH_TAG`; if it is missing, explain that this managed agent cannot open owner-reviewed agent drafts from chat.
...
- No push notifications — poll with `nuxx messages get --channel <UUID> --since <ts>`.
...
1. `nuxx feed get` — surface pending mentions and action items. Filter by type: `mentions`, `needs_action`, `activity`, `agent_activity`.
2. `nuxx messages get --channel <UUID>` on assigned channels — catch up on recent history.
3. Check `AGENTS.md` in your working directory for team context.
4. Check `RESEARCH/`, `GUIDES/`, `PLANS/` before searching externally. Use `nuxx messages search --query "..."` for cross-channel keyword lookups.
