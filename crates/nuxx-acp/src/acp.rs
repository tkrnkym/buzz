...
pub struct AcpClient {
    ...
    ///
    /// Both goose and nuxx-agent emit `session_info_update` with this field;
    ...
    /// run clears (goose/nuxx-agent emit `activeRunId: null` at end of turn).
    ...
    /// deltas. Both goose and nuxx-agent emit this notification; goose gates
    /// on client capability advertisement, nuxx-agent emits unconditionally.
...
/// `_meta.goose.activeRunId`. Emitted by goose and nuxx-agent only.
...
impl AcpClient {
    ...
    ///
    /// Both goose and nuxx-agent emit `session_info_update`; other agents
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        match update_type {
            ...
            "session_info_update" => {
                // Both goose and nuxx-agent emit `session_info_update` with
                ...
                // alongside it at the params level. Goose and nuxx-agent both
...
mod tests {
    ...
    async fn spawn_named_and_read_child_env(
        ...
        let dir = std::env::temp_dir().join(format!("nuxx-acp-env-probe-{}", uuid::Uuid::new_v4()));
    ...
    /// `SessionInfoUpdate` schema), matching what goose and nuxx-agent
    /// emit on the wire.
    ...
    fn capture_path(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join("nuxx-acp-steer-capture");
    ...
    /// Test 1b: no `_meta` at all (goose, nuxx-agent, any older adapter) must
