...
/// name (e.g., `nuxx-acp --verbose models`) are not supported.
...
/// Timeout for `nuxx-acp authenticate`. Browser-based vendor auth can require
...
async fn check_sibling_via_profile(
    ...
    for tag in tags {
        ...
        match nuxx_sdk::nip_oa::verify_auth_tag(&tag_json, &agent_pk) {
            Ok(_) => {
...
/// keyed to `OBSERVER_MAX_PLAINTEXT_LEN` in nuxx-core/observer.rs:25) is what actually
...
/// already-small frame. Reusing that string would mean changing nuxx-core's
/// `encrypt_observer_payload` signature or adding a parallel encrypt path; both
/// are out of this change's scope (nuxx-core stays untouched). The clean `&mut`
...
async fn publish_relay_observer_event(
    ...
    let builder = match nuxx_sdk::build_agent_observer_frame(
        owner_pubkey_hex,
        agent_pubkey_hex,
...
fn handle_relay_observer_control_event(
    ...
    if let Err(e) = nuxx_core::verify_event(&event) {
...
async fn tokio_main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("nuxx_acp=info")),
        )
        .compact()
        .init();
    ...
    {
        tracing::info!("nuxx-acp: setup payload present, entering setup-listener mode");
        return setup_mode::run_setup_listener(config, payload).await;
    }

    tracing::info!("nuxx-acp starting: {}", config.summary());
...
fn try_native_steer(
    ...
    // `queue::format_prompt` uses internally for `[Nuxx event: …]`
...
fn handle_prompt_result(
    ...
    // which live in nuxx-agent's session state and are what `llm: (model) …`
...
fn default_heartbeat_prompt() -> String {
    ...
    format!(
        "[System: Heartbeat]\nTime: {now}\n\n\
         ...
         Your tasks:\n\
         1. Run `nuxx feed get --types needs_action` to check for pending workflow approvals or\n\
            high-priority requests addressed to you.\n\
         2. Run `nuxx feed get --types mentions` to check for unanswered @mentions.\n\
...
/// `nuxx-acp auth-methods` — spawn an adapter, initialize it, print authMethods.
...
/// `nuxx-acp authenticate` — invoke one adapter-owned auth method.
...
mod build_mcp_servers_tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            config_path: std::path::PathBuf::from("./nuxx-acp.toml"),
...
mod observer_payload_trim_tests {
    ...
    fn test_multi_block_prompt_retains_every_section_header_after_elision() {
        ...
        // {text: "[Agent Memory — core]…"}, … {text: "[Nuxx event: …]…<huge>"}].
        ...
        // [Nuxx event] header fell into the elided middle and the count collapsed
        // to 1).
        let sections = [
        ...
        let event_block = texts
            .iter()
            .find(|t| t.starts_with("[Nuxx event: @mention]"))
            .unwrap();
        assert!(
