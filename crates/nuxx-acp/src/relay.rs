...
impl HarnessRelay {
    ...
    pub async fn discover_channels(&self) -> Result<HashMap<Uuid, ChannelInfo>, RelayError> {
        ...
        let member_filter = nostr::Filter::new()
            .kind(Kind::Custom(
                nuxx_core::kind::KIND_NIP29_GROUP_MEMBERS as u16,
            ))
...
/// sends (`crates/nuxx-relay/src/handlers/auth.rs`).
///
...
mod tests {
    ...
    fn make_observer_frame(keys: &Keys) -> Event {
        ...
        let encrypted = nuxx_core::observer::encrypt_observer_payload(
            keys,
            &recipient.public_key(),
            &json!({"type": "test"}),
        )
        .expect("encrypt test observer payload");
        nuxx_sdk::build_agent_observer_frame(
