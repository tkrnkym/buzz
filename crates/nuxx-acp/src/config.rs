//! Configuration for the nuxx-acp harness.
//!
...
/// while running long sub-tools (e.g. a nuxx-agent running another agent, or
...
/// CLI args for `nuxx-acp models` — query available models from an agent.
///
...
/// CLI args for `nuxx-acp auth-methods` — query adapter-advertised login methods.
#[derive(Debug, Parser)]
#[command(
    name = "nuxx-acp auth-methods",
...
/// CLI args for `nuxx-acp authenticate` — start an adapter-owned login flow.
#[derive(Debug, Parser)]
#[command(
    name = "nuxx-acp authenticate",
...
fn default_agent_args(command: &str) -> Option<Vec<String>> {
    match normalize_agent_command_identity(command).as_str() {
        ...
        | "claudecode" | "nuxx-agent" => Some(Vec::new()),
        _ => None,
    }
}
...
/// startup budget (see tkrnkym/nuxx#3355). Skip that unrelated global startup
...
///
/// Codex sandboxes MCP subprocesses (including `nuxx-cli`) behind a Seatbelt sandbox
/// that blocks all outbound network by default. Without this env var, `nuxx-cli`
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        // opens the Seatbelt network sandbox for nuxx-cli (an MCP subprocess). No-op
...
mod tests {
    ...
    fn test_config(mode: SubscribeMode) -> Config {
        Config {
            ...
            config_path: PathBuf::from("./nuxx-acp.toml"),
    ...
    fn test_mentions_mode_default_kinds() {
        ...
        for ch in &channels {
            ...
            assert!(kinds.contains(&nuxx_core::kind::KIND_STREAM_MESSAGE));
            assert!(kinds.contains(&nuxx_core::kind::KIND_WORKFLOW_APPROVAL_REQUESTED));
            assert!(kinds.contains(&nuxx_core::kind::KIND_STREAM_REMINDER));
        }
    }
    ...
    fn default_agent_env_recognizes_hermes_identities() {
        ...
        for command in ["goose", "codex-acp", "claude-agent-acp", "nuxx-agent", ""] {
            assert!(
    ...
    fn codex_network_env_non_codex_agent_returns_none() {
        ...
        assert!(codex_network_env("nuxx-agent", "wss://relay.example.com").is_none());
    }
    ...
    fn test_load_rules_valid_toml() {
        let dir = std::env::temp_dir().join("nuxx-acp-test-valid");
...
"#,
    ...
    fn test_load_rules_empty_name_rejected() {
        let dir = std::env::temp_dir().join("nuxx-acp-test-empty-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_duplicate_name_rejected() {
        let dir = std::env::temp_dir().join("nuxx-acp-test-dup-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_invalid_filter_rejected() {
        let dir = std::env::temp_dir().join("nuxx-acp-test-bad-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    fn test_load_rules_channel_scope_typo_rejected() {
        let dir = std::env::temp_dir().join("nuxx-acp-test-scope-typo");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_too_many_rules_rejected() {
        let dir = std::env::temp_dir().join("nuxx-acp-test-too-many");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_load_rules_filter_too_long_rejected() {
        let dir = std::env::temp_dir().join("nuxx-acp-test-long-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn lazy_pool_defaults_off() {
        let key = "0".repeat(64);
        assert!(!CliArgs::parse_from(["nuxx-acp", "--private-key", &key]).lazy_pool);
    }

    #[test]
    fn lazy_pool_cli_flag_enables_deferred_startup() {
        let key = "0".repeat(64);
        let args = CliArgs::try_parse_from(["nuxx-acp", "--private-key", &key, "--lazy-pool=true"]);
        assert!(args.is_err(), "bool flags do not take an explicit value");
        assert!(CliArgs::parse_from(["nuxx-acp", "--private-key", &key, "--lazy-pool"]).lazy_pool);
    }
    ...
    fn test_multiple_event_handling_default_is_steer() {
        ...
        let args = CliArgs::parse_from(["nuxx-acp", "--private-key", &"0".repeat(64)]);
