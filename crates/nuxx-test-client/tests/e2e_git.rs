...
//!   cargo test -p nuxx-test-client --test e2e_git -- --ignored --nocapture
//! ```
...
use nuxx_media::S3AddressingStyle;
...
fn git_s3_probe_builds_both_addressing_styles() {
    ...
    assert_eq!(path.url(), "https://storage.example/nuxx-media");
    ...
    assert_eq!(virtual_hosted.url(), "https://nuxx-media.storage.example");
}
