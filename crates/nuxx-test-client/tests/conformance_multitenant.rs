...
//! cargo test -p nuxx-test-client --test conformance_multitenant -- --ignored
//! ```
//!
...
mod row_zero_host_binding {
    ...
    /// properties of it (see channel: `nuxx-relay-rewrite`, 2026-06-27):
    ///
    ///   * [`super::channels_membership::same_channel_uuid_in_two_communities_is_isolated`]
    ///     (Mari, `nuxx-db`) asserts **coexistence**: a channel UUID that exists
...
mod nip11_relay_info {
    ...
    /// (`crates/nuxx-relay/src/nip11.rs::_RELAY_INFO_BUILD_STATIC_INPUT_FENCE`):
...
// ---------------------------------------------------------------------------
// API tokens and NIP-98 replay (Sami — nuxx-auth)
// ---------------------------------------------------------------------------
mod api_tokens_nip98_replay {
    ...
    /// in `nuxx-relay`** — there is no route by which a client can bind a token
    /// to a community over HTTP, so the "mint in A, present to B" precondition
    /// has no entry point.
    ///
    /// Verified on PR head:
    ///   * `crates/nuxx-relay/src/router.rs:52-79` — full route list is `/`,
    ...
    ///   * `crates/nuxx-relay/src/api/` directory is `{bridge, events, media,
    ...
    /// contract (its deps are `nuxx-ws-client`/`reqwest`/`tokio-tungstenite`/
    /// `s3` — no `sqlx`, no `nuxx-db`).
    ///
    ...
    ///
    ///   1. `crates/nuxx-db/src/api_token.rs:425
    ...
    ///   2. `crates/nuxx-db/src/api_token.rs:488
    ...
    fn token_minted_in_a_does_not_authorize_in_b() {
        ...
        // If anyone adds a `/tokens` route to `nuxx-relay`, this row's shape
    ...
    /// in `crates/nuxx-relay/src/api/bridge.rs:79` (return `Ok(())` before
    ...
    /// (`crates/nuxx-auth/src/nip98_replay.rs:163
    ...
    /// the pattern in `crates/nuxx-auth/src/nip98.rs`; kept local to this
...
// ---------------------------------------------------------------------------
// Membership / allowlist / archived identities (Sami — nuxx-auth)
// ---------------------------------------------------------------------------
...
mod users_profiles_nip05 {
    ...
    /// `crates/nuxx-relay/src/api/nip05.rs::extract_domain` — strips the
    ...
    /// `crates/nuxx-db/src/user.rs:140` (`UPDATE users SET ... WHERE
    /// community_id = $N AND pubkey = $M`) AND the event-storage fence at
    /// `crates/nuxx-db/src/event.rs::get_events_by_ids` (or the equivalent
    ...
    /// `crates/nuxx-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// validates the handle via `crates/nuxx-relay/src/api/nip05.rs::canonicalize_nip05`
    ...
    /// from `crates/nuxx-db/src/user.rs::get_user_by_nip05` (line 185 form
...
// ---------------------------------------------------------------------------
// Feed read-side isolation (Perci — nuxx-db/relay bridge)
// ---------------------------------------------------------------------------
mod feed_read_side_isolation {
    ...
    /// `crates/nuxx-db/src/feed.rs`; this row is the wire-level conformance
...
// ---------------------------------------------------------------------------
// Channels and channel membership (Mari — nuxx-db)
// ---------------------------------------------------------------------------
mod channels_membership {
    ...
    /// `crates/nuxx-relay/src/handlers/side_effects.rs::handle_create_group`.
    ...
    /// `crates/nuxx-db/src/event.rs::query_events` (the non-p-tag branch,
...
mod workflows {
    ...
    /// `crates/nuxx-relay/src/handlers/command_executor.rs:703`
    ...
    /// (`crates/nuxx-workflow/src/lib.rs` — "approval gates not yet implemented,
    ...
    /// minting), **not** nuxx-db — the scoping fence it will exercise is
    /// already landed.
    #[tokio::test]
    #[ignore]
...
mod search_fts {
    ...
    ///   - `crates/nuxx-search/src/query.rs::search` lines 160-161 (the FTS
    ///     `WHERE community_id = $ctx` predicate); and
    ///   - `crates/nuxx-db/src/event.rs::get_events_by_ids` lines 870-872
...
// ---------------------------------------------------------------------------
// Redis pub/sub, presence, typing, cache invalidation (Max — nuxx-pubsub)
// ---------------------------------------------------------------------------
mod pubsub_presence_typing {
    ...
    ///     `nuxx-pubsub/src/presence.rs::presence_key` (shared Redis key). The B
...
// ---------------------------------------------------------------------------
// Media / Blossom / S3 (Perci — nuxx-media)
// ---------------------------------------------------------------------------
mod media_blossom {
    use super::*;
...
// ---------------------------------------------------------------------------
// Git hosting / NIP-34 / object storage (Perci — nuxx-media/git)
// ---------------------------------------------------------------------------
mod git_hosting {
    use super::*;
...
// ---------------------------------------------------------------------------
// Audit log and observability (Dawn — nuxx-audit)
// ---------------------------------------------------------------------------
mod audit_log {
    ...
    //! `crates/nuxx-relay/src/router.rs` (the route list is `/`, `/info`,
    ...
    //! `nuxx_audit::AuditService::{verify_chain, get_entries}`, which are
    //! operator-internal (consumed by `nuxx-admin`). `crates/nuxx-audit/src/
    ...
    //! (its deps are `nuxx-ws-client`/`reqwest`/`tokio-tungstenite`/`s3` — no
    //! `sqlx`, no `nuxx-audit`), and a DB-direct read can never catch a
    ...
    //!      `nuxx_audit::service::tests::chains_are_independent_per_community`
    ...
    //!      `nuxx_relay::handlers::event::tests::
    ...
    //!   2. **Errors don't leak** —
    //!      `nuxx_audit::error::tests::audit_error_text_carries_no_community_id_or_constraint`
    //!      asserts no `AuditError` variant's rendered text embeds a
    //!      `community_id`, constraint name, or cross-community object id.
    //!
    //! Substrate on PR head: `crates/nuxx-audit/src/entry.rs` keys `AuditEntry`
