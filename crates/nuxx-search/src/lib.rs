#![deny(unsafe_code)]
#![warn(missing_docs)]
//! Nuxx search — community-scoped Postgres full-text search over Nuxx events.
//!
...
//! by `nuxx-db`. The relay refetches canonical events through `nuxx-db`'s
