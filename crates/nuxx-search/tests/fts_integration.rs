...
use nuxx_search::{ChannelScope, SearchQuery, SearchService};
...
async fn setup() -> (PgPool, String) {
    let url = std::env::var("NUXX_TEST_DATABASE_URL").unwrap_or_else(|_| TEST_DB_URL.to_string());
...
async fn kind0_search_by_display_name_works_without_flattening() {
    ...
    insert_event(
        ...
        pk,
        0,
        r#"{"display_name":"Alice Wonderland","name":"alice","nip05":"alice@nuxx.app","about":"hello"}"#,
        None,
        1700000000,
    )
    .await;
    ...
    let svc = SearchService::new(pool.clone());
    for q in ["wonderland", "alice", "alice@nuxx.app"] {
        let r = svc
            .search(&SearchQuery {
                community: c,
                q: q.to_string(),
...
async fn prefix_mode_matches_final_token_prefix_without_changing_full_text() {
    ...
    let prefix = svc
        .search(&SearchQuery {
            ...
            page: 1,
            per_page: 10,
            mode: nuxx_search::SearchMode::Prefix,
        })
        .await
...
async fn p_gated_persistent_kinds_have_storage_null_tsvector() {
    ...
    let persistent: Vec<u32> = P_GATED_KINDS
        .iter()
        .copied()
        .filter(|&k| !nuxx_core::kind::is_ephemeral(k))
        .collect();
    assert!(
        !persistent.is_empty(),
