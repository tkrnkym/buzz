...
/// sends `notifications/cancelled` to the MCP server. With nuxx-dev-mcp,
...
async fn cancel_kills_inflight_tool_via_mcp_notification() {
    // nuxx-dev-mcp is a separate crate; locate its binary relative to
    // the nuxx-agent test binary (they share the same target dir).
    let self_bin = std::path::PathBuf::from(env!("CARGO_BIN_EXE_nuxx-agent"));
    let dev_mcp_bin = self_bin.parent().unwrap().join("nuxx-dev-mcp");
    ...
    if !dev_mcp_is_executable {
        eprintln!(
            "SKIP: nuxx-dev-mcp not built at {}; run `cargo build -p nuxx-dev-mcp` first",
            dev_mcp_bin.display()
        );
        return;
    }
...
/// any MCP server (not just nuxx-dev-mcp) when a session is cancelled
