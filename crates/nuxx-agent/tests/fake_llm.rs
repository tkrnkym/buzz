//! Integration test: fake LLM HTTP server + nuxx-agent subprocess.
//!
...
impl Harness {
    async fn spawn(base_url: &str) -> Self {
        ...
        cmd.env("NUXX_AGENT_PROVIDER", "openai")
            ...
            .kill_on_drop(true);
        let mut child = cmd.spawn().expect("spawn nuxx-agent");
...
async fn init_session(h: &mut Harness) -> String {
    ...
    assert_eq!(r["result"]["agentInfo"]["name"], "nuxx-agent");
...
async fn system_prompt_reaches_llm_system_role() {
    ...
    let default_prompt = "You are nuxx-agent";
    assert!(
...
async fn system_prompt_absent_no_canary() {
    ...
    assert!(
        system_content.contains("You are nuxx-agent"),
...
/// Wait for the `activeRunId` advert nuxx-agent emits at prompt start and
...
/// completion_tokens). nuxx-agent maps these to `accumulatedInputTokens` /
...
/// nuxx-agent must treat this turn's total as Unknown and poison the cumulative.
...
/// nuxx-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, nuxx-agent must
...
/// (so token counts are observed), nuxx-agent must still emit the usage
