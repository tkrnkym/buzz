...
//! exercised manually via the `nuxx-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `nuxx-agent` binary with
...
use nuxx_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real nuxx-agent binary with `DATABRICKS_TOKEN` set (so the
