...
/// one. `expected_run_id` must match the run id nuxx-agent advertised via
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) are unexpected — nuxx-agent
...
/// method goose uses for custom usage and status events.  Used by nuxx-agent
/// to emit the `usage_update` payload so nuxx-acp's `UsageTracker` can treat
/// nuxx-agent and goose symmetrically.
