...
/// `cache_namespace` is the directory under `~/.config/nuxx-agent/oauth/`
...
pub struct PkceOAuthConfig {
    ...
    /// `~/.config/nuxx-agent/oauth/<cache_namespace>/`. Production code
...
impl PkceOAuthTokenSource {
    ...
    pub(crate) async fn try_bearer_no_browser(&self) -> Result<String, AgentError> {
        ...
        Err(AgentError::LlmAuth(
            "no cached Databricks token; run `nuxx-agent auth databricks` first".into(),
        ))
    }
}
...
mod tests {
    ...
    fn cache_path_includes_namespace_and_hash() {
        ...
        assert!(p.to_string_lossy().contains("/nuxx-agent/oauth/demo/"));
