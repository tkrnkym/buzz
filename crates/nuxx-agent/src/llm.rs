...
/// consent for `databricks-cli` covers nuxx-agent too.
...
async fn openrouter_post(
    ...
    for attempt in 0..MAX_RETRIES {
        let resp = match http
            .post(url)
            .header("content-type", "application/json")
            .header("HTTP-Referer", "https://github.com/tkrnkym/nuxx")
...
mod tests {
    ...
    async fn openrouter_post_sends_attribution_headers() {
        ...
        let header_str = headers
            ...
            .to_lowercase();
        assert!(
            header_str.contains("http-referer: https://github.com/tkrnkym/nuxx"),
            "got: {header_str}"
        );
        assert!(
