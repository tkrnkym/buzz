//! git-credential-nostr — NIP-98 git credential helper for Nuxx.
//!
...
/// Load the NIP-OA owner attestation injected by Nuxx Desktop/ACP.
///
...
fn parse_method(wwwauth: &str) -> Option<HttpMethod> {
    ...
    // Handles variations: `Nostr method="GET", realm="nuxx"` and
    // `Nostr method="GET",realm="nuxx"` (with or without space after comma).
...
pub fn run() -> i32 {
    ...
    // No Nostr challenge from the server — this isn't a Nuxx remote.
    // Exit silently so git falls through to the next credential helper.
    // This check comes FIRST so non-Nuxx remotes never hit validation errors.
