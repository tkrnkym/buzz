...
require_canonical_repository() {
  ...
  origin_url="$(git config --get remote.origin.url 2>/dev/null)" || \
    fail_release_ruleset "origin is required and must point to tkrnkym/buzz" || return 1
  case "$origin_url" in
    git@github.com:tkrnkym/buzz.git|ssh://git@github.com/tkrnkym/buzz.git|https://github.com/tkrnkym/buzz.git|https://github.com/tkrnkym/buzz)
      ;;
    *)
      fail_release_ruleset "origin must point to canonical tkrnkym/buzz, not '$origin_url'" || return 1
      ;;
  esac
}
...
require_release_tag_ruleset() {
  ...
  ruleset_endpoint="repos/tkrnkym/buzz/rulesets/$RELEASE_TAG_RULESET_ID"
