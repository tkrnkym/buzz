...
require_canonical_repository() {
  ...
  origin_url="$(git config --get remote.origin.url 2>/dev/null)" || \
    fail_release_ruleset "origin is required and must point to tkrnkym/nuxx" || return 1
  case "$origin_url" in
    git@github.com:tkrnkym/nuxx.git|ssh://git@github.com/tkrnkym/nuxx.git|https://github.com/tkrnkym/nuxx.git|https://github.com/tkrnkym/nuxx)
      ;;
    *)
      fail_release_ruleset "origin must point to canonical tkrnkym/nuxx, not '$origin_url'" || return 1
      ;;
  esac
}
...
require_release_tag_ruleset() {
  ...
  ruleset_endpoint="repos/tkrnkym/nuxx/rulesets/$RELEASE_TAG_RULESET_ID"
