...
grep -q '^BUNDLE_IDENTIFIER = com\.nuxx\.nuxxMobile\.feature-work-1$' "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = com\.nuxx\.nuxxMobile$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production bundle identifier" \
  || fail "Release.xcconfig must keep BUNDLE_IDENTIFIER = com.nuxx.nuxxMobile"
...
if [[ "$3 $4 $5" == "shell pm list" ]]; then
  printf 'package:xyz.block.nuxx.mobile\n'
  printf 'package:xyz.block.nuxx.mobile.feature_work_1\n'
  printf 'package:xyz.block.nuxx.mobile.w_2fast\n'
...
if printf '%s\n' "$clean_out" | grep -Eq '(would uninstall|uninstalling).*xyz\.block\.nuxx\.mobile$'; then
