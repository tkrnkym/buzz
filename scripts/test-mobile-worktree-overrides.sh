...
if [[ "$3 $4 $5" == "shell pm list" ]]; then
  printf 'package:ai.nuxx.mobile\n'
  printf 'package:ai.nuxx.mobile.feature_work_1\n'
  printf 'package:ai.nuxx.mobile.w_2fast\n'
...
if printf '%s\n' "$clean_out" | grep -Eq '(would uninstall|uninstalling).*ai\.nuxx\.mobile$'; then
