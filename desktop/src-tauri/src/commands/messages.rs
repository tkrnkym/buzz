...
pub async fn get_feed(
    ...
    let mut mention_filter = serde_json::json!({
        "kinds": [
            ...
            45001,
            45003,
            nuxx_core_pkg::kind::KIND_GIT_PULL_REQUEST,
            nuxx_core_pkg::kind::KIND_GIT_PR_UPDATE,
            nuxx_core_pkg::kind::KIND_GIT_ISSUE,
            nuxx_core_pkg::kind::KIND_GIT_STATUS_OPEN,
            nuxx_core_pkg::kind::KIND_GIT_STATUS_MERGED,
            nuxx_core_pkg::kind::KIND_GIT_STATUS_CLOSED,
            nuxx_core_pkg::kind::KIND_GIT_STATUS_DRAFT,
        ],
        "#p": [my_pubkey],
        "limit": cap,
    });
...
pub async fn get_event(event_id: String, state: State<'_, AppState>) -> Result<String, String> {
    let events = query_relay(
        ...
        &[serde_json::json!({
            "ids": [event_id],
            "kinds": [0, 1, 3, 5, 7, 9, 30078, 40002, 40003, 40008, 40099, 40100, 45001, 45003, nuxx_core_pkg::kind::KIND_HUDDLE_STARTED],
            "limit": 1
        })],
    )
    .await?;
...
async fn resolve_thread_ref(
    ...
    let evs = query_relay(
        ...
        &[serde_json::json!({
            "ids": [parent_event_id],
            "kinds": [9, 40002, 45001, 45003, nuxx_core_pkg::kind::KIND_HUDDLE_STARTED],
            "limit": 1
        })],
    )
    .await?;
...
pub async fn send_channel_message(
    ...
    let kind_num = kind.unwrap_or(nuxx_core_pkg::kind::KIND_STREAM_MESSAGE);

    let mut resolved_root: Option<String> = None;

    let builder = match kind_num {
        nuxx_core_pkg::kind::KIND_FORUM_POST => events::build_forum_post(
            channel_uuid,
            content.trim(),
            &mention_refs,
            &media,
        ...
        nuxx_core_pkg::kind::KIND_FORUM_COMMENT => {
...
async fn find_managed_agent_channel_message_by_marker(
    ...
    for _ in 0..10 {
        let mut filter = serde_json::json!({
            "kinds": [nuxx_core_pkg::kind::KIND_STREAM_MESSAGE],
            "#h": [channel_id],
            "limit": 500,
        });
...
fn legacy_managed_agent_auth_tag(
    ...
    nuxx_sdk_pkg::nip_oa::compute_auth_tag(owner_keys, agent_pubkey, "")
        .map(Some)
...
pub async fn add_reaction(
    ...
    let builder = match emoji_url {
        ...
        Some(url) => nuxx_sdk_pkg::build_custom_emoji_reaction(target_eid, emoji.trim(), &url)
