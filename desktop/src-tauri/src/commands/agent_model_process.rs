...
use crate::managed_agents::{
    build_nuxx_agent_provider_defaults, default_agent_workdir, known_acp_runtime,
...
pub(super) async fn run_agent_models_command(
    ...
    let output = tokio::task::spawn_blocking(move || {
        ...
        cmd.stdout(std::process::Stdio::piped())
            ...
            .output()
            .map_err(|e| format!("failed to spawn nuxx-acp models: {e}"))
    })
    .await
    ...
    if !output.status.success() {
        ...
        return Err(format!(
            "nuxx-acp models failed (exit {}): {stderr_redacted}",
