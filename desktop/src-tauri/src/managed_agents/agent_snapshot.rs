//! `nuxx-agent-snapshot v1` — manifest type, encoder, and decoder stubs.
//!
...
//!   - `.agent.png` — avatar image with manifest in a `nuxx_agent_snapshot`
//!     tEXt chunk
//!
...
pub const PNG_CHUNK_KEYWORD: &str = "nuxx_agent_snapshot";
...
pub const FORMAT_DISCRIMINATOR: &str = "nuxx-agent-snapshot";
...
/// The top-level `nuxx-agent-snapshot v1` manifest.
///
/// Serializes to / from JSON. Embedded in `.agent.json` directly, or in the
/// `nuxx_agent_snapshot` tEXt chunk of a `.agent.png` (base64-encoded).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
...
/// in the `nuxx_agent_snapshot` tEXt chunk.
...
pub fn decode_snapshot_png(png_bytes: &[u8]) -> Result<AgentSnapshot, String> {
    ...
    let chunk_text = info
        ...
        .map(|c| c.text.as_str())
        .ok_or_else(|| "PNG does not contain a nuxx_agent_snapshot tEXt chunk".to_string())?;
