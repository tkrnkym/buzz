...
fn validate_harness_definition(def: &HarnessDefinition) -> Result<(), String> {
    ...
    // nuxx-acp side), so a literal comma inside one argument would silently
...
fn builtin_ids() -> impl Iterator<Item = &'static str> {
    const TIER1: &[&str] = &["goose", "claude", "codex", "nuxx-agent"];
...
// spawn time, causing silent fallback to nuxx-agent.
//
...
// registry, the caller gets a typed error — never a silent nuxx-agent fallback.
...
mod tests {
    ...
    fn builtin_ids_are_rejected() {
        ...
        for id in &["goose", "claude", "codex", "nuxx-agent"] {
